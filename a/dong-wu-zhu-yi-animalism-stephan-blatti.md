...
1. *x*（构成对象）在事件中非派生地实例化了_F_，即使它不构成_y_（被构成对象）也会实例化_F_；
2. *y_在事件中非派生地实例化了_F*，如果_x_不构成_y_，则_x_将无法实例化_F_；并且
