...
| 1*. | 必然地，如果任何 x 是上帝，那么对于与 x 不同的每个具体对象来说，x 的活动是其存在的因果必要条件，而且如果实际上存在与 x 不同的一个或多个有限存在，那么 x 的活动对于至少其中一个的存在来说是因果充分（在强义上）。 |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| 1*. | 必然地，如果任何 x 是上帝，那么对于与 x 不同的每个具体对象，x 的活动是其存在的因果必要条件，如果实际上存在一个或多个与 x 不同的有限存在，那么 x 的活动对于至少其中一个的存在是因果充分条件（在强意义上）。 |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
21. 有两个神和可能的有限存在是不可能的，也就是说，如果可能存在有限存在，那么存在两个神是错误的。（从 1*到 20。如果一个命题[例如，16]与一个或多个必然真理[例如，1*]一起蕴含另一个[例如，19]，并且第二个是不可能的，那么第一个是不可能的。）
