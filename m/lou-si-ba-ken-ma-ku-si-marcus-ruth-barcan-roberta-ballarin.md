...
(∀α1)(∀α2)…(∀αn)(Γ⇔E) (其中α1, α2, …, αn 是Γ和 E 中的所有自由变量) 并且 B 是通过将 E 替换为 A 中一个或多个Γ的出现而得到的结果，则可以从 A 推导出 B，从 B 推导出 A。 4. 概括：如果 B 是通过将个体变量β替换为 A 中所有自由出现的α而得到的结果，则可以从 A 推导出(∀β)B。
