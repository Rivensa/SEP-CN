# 不可通约性价值 incommensurable (Nien-hê Hsieh and Henrik Andersson)
...
第 1 节的其余部分考虑了提出的以不可通约性来构思价值的方式。

### 1.3 价值不可通约性的构思

本节概述了三种价值不可通约性的构思，每一种都捕捉到了不可通约性是由于缺乏共同度量的某种意义上的情况。

第一个概念以限制一个价值的进一步实现如何超越另一个价值的实现来描述价值的不可通约性。詹姆斯·格里芬提出了这种形式的价值不可通约性。其中一种形式涉及他所称的“压倒”。在价值 A 和 B 之间的冲突中，如果“任何数量的 A，无论多么小，都比任何数量的 B 更有价值”（格里芬 1986 年，83 页），则称 A 压倒 B。价值不可通约性的一种较弱形式涉及格里芬所称的“不连续性”。如果两个价值 A 和 B 在这个意义上是不可通约的，那么“只要我们拥有足够的 B，任何数量的 A 都超过任何进一步的 B 数量；或者足够的 A 超过任何数量的 B”（格里芬 1986 年，85 页）。

如果价值在这个第一个意义上是不可通约的，那么实现一个价值是否超过实现另一个价值就没有歧义。然而，许多理论家认为，实现一个价值是否超过实现另一个价值存在歧义，这被认为是不可通约价值的一个核心特征。价值不可通约性的第二和第三个概念旨在捕捉这一特征。

根据第二个概念，只有当没有真正的整体排序可以对比实现一个价值与实现另一个价值时，价值才是不可通约的。例如，大卫·威金斯将这一点提出为价值不可通约性的一个概念。他写道，如果“在选择和比较的整个范围中，A 和 B 在其中发挥作用的情况下，没有一种普遍的方式可以交换 A 和 B”（1997 年，59 页），则两个价值是不可通约的。

这种对不可通约性的第二种理解否认了亨利·理查森所称的“强通约性”（1994 年，104-105）。强通约性是这样的命题：在所有价值冲突中，存在一个真正的排名，将一个价值的实现与另一个价值的实现相对应，以一个共同的价值为基础。然而，对于这样一个独特的共同价值的否认，并不排除理查森所称的“弱通约性”（1994 年，105）。弱通约性是这样的命题：在任何给定的价值冲突中，存在一个真正的排名，将一个价值的实现与另一个价值的实现相对应，以某个价值为基础。这个价值可以是所讨论的价值之一，也可以是某个独立的价值。这个价值在不同的价值冲突中也可能不同。对于强通约性的否认并不意味着对于弱通约性的否认。即使没有系统或一般的方法来解决任何给定的价值冲突，也可能存在某个价值，使得一个价值的实现相对于另一个价值的实现进行排名。唐纳德·里根为强通约性的命题进行了辩护（里根 1997 年）。

对于价值不可通约性的第三种理解否认了强通约性和弱通约性（理查森 1994 年，威金斯 1997 年，威廉姆斯 1981 年）。这种理解声称，在某些价值冲突中，不存在真正的价值排名。

这种关于价值不可通约性的第三种观念有时被认为是解释为什么在价值冲突中，一个价值的增益并不总是能够抵消另一个价值的损失的必要条件。这种观点假设，只要在实现一个价值和实现另一个价值之间存在真正的排序，一个价值的增益就能够抵消另一个价值的损失。许多评论者对这种假设表示怀疑。本条目留下了这样一种可能性，即当一个价值和另一个价值之间存在真正的排序时，一个价值的增益不一定能够抵消另一个价值的损失。

如果我们接受这种可能性，就会引发一系列问题。一个问题是，如果一个价值的增益不能抵消另一个价值的损失，那么价值实现的排序意味着什么。第二个问题是，价值不可通约性的第一种和第二种观念是否各自有两个版本：一个版本中，一个价值的增益能够抵消另一个价值的损失，另一个版本中则不能。第三个问题涉及价值不可通约性与悲剧之间的关系。人们可能认为，选择成为悲剧的原因是无论选择哪个替代方案，价值的增益都无法抵消另一个价值的损失。然而，并非所有作者都认为涉及不可通约价值的所有价值冲突都是悲剧性的（Richardson 1994, 117）。例如，Wiggins 将价值不可通约性的第二种观念保留给他所称的“普通的或花园式不可通约”选择，将第三种观念保留给他所称的“具有特殊环境和悲剧性的不可通约”选择（1997, 64）。

## 2. 不可比较性

最近的研究不再关注抽象价值之间的可比性，而是关注具体价值的不可比性，通常是在选择的背景下进行研究（Broome 1997, 2000; Chang 1997, 2002; Griffin 1986; Raz 1986）。在像 Joseph Raz 在第 1.2 小节中描述的职业选择的例子中，价值的承载者有时会显得不可比较。

在这些例子中，不可比较性的论证部分依赖于 Ruth Chang 所称的“小幅改进论证”（Chang 2002b, 667）。正如在例子的初始讨论中所指出的，如果法律和音乐职业的价值相等，那么稍微改进的法律职业将比音乐职业更好，但这个判断似乎是错误的。小幅改进论证的一般形式如下：“如果（1）A 在 V 方面既不比 B 更好也不比 B 更差，（2）A+在 V 方面比 A 更好，（3）A+在 V 方面不比 B 更好，那么（4）A 和 B 之间没有任何标准的三分关系（相对于 V 而言）”，其中 V 代表比较的相关考虑因素（Chang 2002b, 667–668）。除了 Raz 之外，Derek Parfit 和 Walter Sinnott-Armstrong 也是提出小幅改进论证的人之一（Parfit 1984; Sinnott-Armstrong 1985）。然而，对于偏好关系的类似论证却有着更长的历史。Leonard J. Savage 在 1954 年已经暗示了这种可能性，1958 年 R. Duncan Luce 提出了一个版本的论证，他将其归因于 Howard Raiffa，Ronald de Sousa 在 1974 年也对此进行了讨论（Savage 1954; Luce 1958; de Sousa 1974）。

关注价值的不可比较性引发了文献中的两个研究方向。第一个关注不可比较性与模糊性之间的关系；一些人认为所谓的不可比较性实际上更好地理解为模糊性。第二个关注两个物品之间可能存在的比较关系的范围；一些人认为小幅改进论证并没有证明不可比较性，而是证明了存在比以前认为的更多的价值关系。本节总结了每个研究方向内的辩论。

### 2.1 模糊性

Raz 将不可比较性与他所称的“价值的不确定性”区分开来。回想一下，Raz 将两个价值承担者定义为不可比较的，当且仅当“要么一个比另一个更好，要么它们的价值相等”不成立。价值的不确定性是模糊性的一种情况：对于两个物品，“要么一个比另一个更好，要么它们的价值相等”既不是真的也不是假的。Raz 认为价值的不确定性是由于“语言的普遍不确定性”而产生的（1986 年，324 页）。

相比之下，其他哲学家认为，将不可比较性的所谓例子解释为模糊性（Griffin 1986, 96; Broome 1997, 2000; Andersson 2017; Elson 2017; Dos Santos 2019）是有道理的。有人认为，小幅改进论证未能排除价值承担者之间关系不确定的可能性；它们可能通过标准的三分关系之一相互关联，但具体是哪一种关系可能是不确定的（Wasserman 2004; Klockseim 2010; Gustafsson 2013）。

对于模糊解释，还有更多正面的论据支持。例如，布鲁姆引入了他所称的“标准配置”（1997 年，96 页；2000 年，23 页）。想象一下 Raz 的例子中的音乐和法律职业。将音乐职业固定为“标准”。现在想象一下法律职业的变化，排列成一条线，其中在一个方向上，这些变化比标准更好，而在另一个方向上，标准比这些变化更好。存在一个法律职业的中间区域，这些职业既不比标准更好，也不比法律职业更好。如果这个区域只包含一个项目，布鲁姆将这个法律职业定义为与标准一样好。如果这个区域包含多个项目，那么这个区域要么是“硬模糊性”，要么是“软模糊性”。一个区域不能既是硬模糊性又是软模糊性。在硬模糊性的区域中，法律职业不比标准更好，标准也不比法律职业更好（1997 年，73 页，76 页）。在软模糊性的区域中，法律职业既不是真的也不是假的比标准更好，标准也既不是真的也不是假的比法律职业更好（1997 年，76 页）。后者是一个模糊的区域。布鲁姆认为，包括“比...更好”在内的不确定比较是软模糊的，因此不存在硬模糊性，也没有小幅改进论所暗示的可能性的空间。

通过理解不可比较性涉及模糊性，布鲁姆与拉兹（Broome 2000, 30）持不同意见。拉兹将不可比较性定义为与模糊性相容，但并不意味着涉及模糊性。格里芬也认为不可比较性涉及模糊性（1986, 96）。布鲁姆与格里芬的分歧在于软性不确定性区域的宽度和重要性。布鲁姆认为格里芬暗示如果存在软性不确定性区域，那么它是狭窄且不重要的。布鲁姆认为模糊性不必意味着狭窄或缺乏重要性（2000, 30–31）。
...
这个论点受到了质疑。正如 Ruth Chang 指出的那样，通过引用约翰·斯图尔特·密尔，价值可能具有定性和定量两个方面，原则上允许价值承载者由于其定性特征的差异而无法比较。（Chang 1997b，16-17）。
...
价值不可通约性也在关于无意志行为的辩论中起到了作用（Nussbaum 2001, 113–117）。例如，大卫·威金斯（David Wiggins）引用了价值不可通约性的概念来暗示“欲望满足和评价的心理源泉的多样性”（1998, 266）。根据威金斯的观点，这种多样性使得我们能够对行为体对不是最好的事物的吸引力给出一个连贯的解释。它允许欲望和价值之间存在差异，使得无意志的个体可以被吸引到在那个时刻不应该追求的价值上。威金斯引用了价值不可通约性来捕捉这样一个观念：在追求不应该追求的价值时所获得的收益并不能减少选择不是最好的事物所带来的损失。
...
在第 4.1 小节中表达的优化和最大化之间的区别所表达的思想可以更一般地用理性资格的概念来表达。说一个选择在理性上是合格的，意味着选择它不是一个无理由的选择。哪些选择被判断为理性合格可能因合理选择理论而异。根据最大化作为合理选择理论，一个选择只有在没有更好的选择时才是理性合格的。
...
斯托克的合理选择解释与经济学和理性选择文献中使用的“满足”概念不同。满足是赫伯特·西蒙（1955）引入经济学文献中的选择过程，当行为体找到一个“足够好”的选择时，停止寻找其他选择是理性的（拜伦，2004）。满足与斯托克的解释不同之处在于，满足是基于一种更普遍的最大化选择解释而具有工具性合理性。例如，在寻找最佳选择代价过高或不可能的情况下，满足选择变得合理。相比之下，根据斯托克的解释，选择“足够好”的替代选择是非工具性合理的（斯托克，2004）。
