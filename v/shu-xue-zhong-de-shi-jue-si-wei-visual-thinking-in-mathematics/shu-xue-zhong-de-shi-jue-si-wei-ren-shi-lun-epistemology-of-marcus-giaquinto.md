...
我们在这里关心的是通过证明的步骤进行思考，无论是第一次（第一次成功尝试构建证明）还是按照给定的证明。显然，我们希望区分仅伴随证明步骤思考过程的视觉思维和对过程至关重要的视觉思维。这并不总是一件简单的事情，因为证明可以以不同的方式呈现。不同的呈现方式可以有多不同，但仍然是相同证明的呈现方式吗？对此没有上下文无关的答案。通常，数学家们乐意将两种呈现方式视为呈现相同证明，如果两种情况下的中心思想是相同的。但是，如果一个人的主要关注点是思考证明所涉及的内容，仅有中心思想是不足以个体化它的：整体结构、步骤的顺序以及可能影响涉及的认知过程的其他因素将是相关的。

一旦证明的个体化问题解决了，我们就可以区分可替代思维和多余思维，其中这些属性是相对于给定的论证或证明来理解的。在思考一个证明的过程中，如果在一个被认为是通过同一个证明进行思考的过程中，某种其他类型的思维可以代替给定部分的思维，那么给定部分的思维是可替代的。如果在没有替代的情况下切除给定部分的思维，那么这将是一个通过同一个证明进行思考的过程。多余的思维可能在促进对证明文本的理解和使人理解证明步骤背后的思想方面非常有价值，但对于通过证明进行思考来说并非必要。

毫无争议的是，在符号操作中涉及的视觉思维，例如在关于群的基本引理的证明的“代数”步骤中，可以是必要的，即既不是多余的也不是可替代的。问题在于使用图表进行视觉思维，其中“图表”广泛用于包括所有非符号化的视觉表达。让我们同意，在通过证明进行思考的过程中可能存在多余的图表思维。这留下了几种可能性。

* (a)在通过证明进行思考的过程中，所有图表思维都是多余的。
* (b)在思考证明过程中，并非所有的图解思维都是多余的；但如果不是多余的，它可以被非图解思维所取代。
* (c)在思考证明过程中，一些图解思维既不是多余的，也不能被非图解思维所取代。

### 3.1 可靠性问题

早先提出的负面观点认为，图表在证明中没有作用，这意味着主张（a）。主张（a）背后的想法是，由于图表推理是不可靠的，如果一个思考过程中包含一些非多余的图表思维，那么这个过程缺乏认识论安全性，不能被视为一个思考证明的案例。

这种观点，特别是主张（a），受到了一些非视觉上证明图表思维可靠性的案例的威胁。最清晰的例子是在一个形式系统中，其句法对象中的公式被图表取代，并且推理规则之间存在类型的图表转换。假设你在这样一个形式系统和其解释中进行思考，然后通过一个关于该解释的系统完备性的证明来思考该系统的正确性；假设你随后检查一系列图表，逐步验证其构成了系统中的推导；最后假设你恢复解释以得出结论。（顺序不重要：可以先进行推导，然后再进行正确性证明。）整个过程都构成了对结论进行思考的证明；而涉及的图表思维并非多余。

Shin 等人（2013）报告说，逻辑和几何的形式图表系统已被证明是正确的。人们确实在这些系统中进行了证明。这足以反驳主张（a），即在思考证明过程中所有的图表思维都是多余的主张。作为一个具体的例子，图 1 展示了欧几里得几何的一部分的形式图表系统中欧几里得第一个定理的推导（Miller 2001）。

![a three by three  array of rectangles each containing a diagram Going left to right then top to bottom, the first has a line segment with each end having a dot  The second is a circle with a radius drawn and dots on each end of the radius line segment The third is the same the second except another overlapping circle is drawn using the same radius line segment but with the first circle's center dot now on the perimeter and the first circle's perimeter dot now the center of the second circle, dots are added at the two points the circles intersect  The fourth diagram is identical to the third except a line segment is drawn from the top intersection dot to the first circle's center dot  The fifth diagram is like the fourth except a line segment is drawn from the top intersection dot to the center  dot of the second circle  ](https://plato.stanford.edu/entries/epistemology-visual-thinking/fig1.png)

 图 1

关于 Tennant 声称证明是“仅由句子组成的句法对象”而不是图表的说法如何？证明从不是句法对象。单独的形式推导是句法对象，但不是证明。在没有对形式系统语言的解释的情况下，推导的最终公式毫无意义；因此没有证明。如果没有对系统相对于解释的合理性进行证明，人们可能缺乏足够的理由相信所有可推导的结论都是真实的。形式推导加上解释和合理性证明可以成为推导结论的证明，但整个过程不是句法对象。此外，真正是句法对象的证明部分，形式推导，不一定仅由句子组成；它可以由图表组成。

在解决了主张（a）之后，再考虑主张（b）：在通过一个证明的思考过程中，并非所有图表思维都是多余的，所有非多余的图表思维都可以在通过同一个证明的思考过程中被非图表思维所替代。使用米勒的形式系统跟随欧几里得第一定理的证明的视觉思维包括通过一系列图表进行，并在每一步中看到下一个图表是由前一个图表的允许变化得到的。很明显，在被视为通过这个证明的思考过程中，图表思维既不是多余的，也不能被非图表思维所替代。这排除了（b），只剩下（c）：在通过证明的思考过程中，涉及图表的某些思维既不是多余的，也不能被非图表思维所替代（而不改变证明）。

### 3.2 非正式证明中的视觉手段

数学实践几乎从不通过形式系统进行。在非形式图示系统的背景下，使用图示被广泛认为是不可靠的。图示可能对所描述的构造不忠实：它可能代表着被描述所排除的某种属性，或者没有被描述所要求的某种属性。这在著名的所有三角形都是等腰三角形的命题的论证中得到了证明：角平分线与对边的垂直平分线的交点被表示为在三角形内部，而实际上应该在外部（Rouse Ball 1939; Maxwell 1959）。这种错误相对较少，通常可以通过一点小心避免，并不是图示本质上固有的；因此，它们不足以支持一般性的不可靠指控。

主要的错误类型是不合理的概括。通常，图表（以及其他非语言的视觉表达）并不将其对象表示为具有被对象的意图或规范排除的属性。但是，图表经常将其对象表示为具有虽然不被规范排除但并不要求的属性。口头描述可以是离散的，因为它们提供的信息不超过所需。但是，视觉表达通常是不离散的，因为它们提供了过多的细节。这通常是不可避免的，因为对于许多属性或种类 F，视觉表达无法在不以特定方式表示为 F 的情况下表示其为 F。例如，任何一个三角形的图表都必须将其表示为具有三个锐角或仅具有两个锐角，即使这两个属性都不是规范所要求的，如果规范是“设 ABC 为一个三角形”。因此，在使用图表推理关于类 K 的任意实例时，我们有可能会不知不觉地依赖于图表中表示的一个不是所有类 K 实例共有的特征。因此，不合理的概括的风险是使用许多图表时固有的危险。

图表的不离散性不仅限于几何图形。古代数学中的点或鹅卵石图表用于证明数论的基本真理，必然显示特定数量的点，尽管这些真理是普遍的。这是一个例子，用于证明第 n 个三角数的公式，即前 n 个正整数的和。

![a grid of blue dots 5 wide and 7 deep, on the right side is a brace embracing the right column labeled n1 and on the bottom a brace embracing the bottom row labeled n](https://plato.stanford.edu/entries/epistemology-visual-thinking/fig2.png)

 图 2

得出的结论是，对于任何正整数 n，从 1 到 n 的整数之和是(n×n+1)/2，但图表展示的是 n=6 的情况。当我们仅仅想象相关类型的显示时，也许可以避免表示特定数量的点；或者如果表示了特定数量，我们的经验可能不会让我们意识到这个数量——就像当我们想象一个星空的夜晚时，我们并不知道确切地有多少颗星星被表示出来。即便如此，可能还是会有一些额外的具体性。例如，在想象刚刚展示的点阵时，人们不太可能只想象两列三个点，即 n=2 的矩形点阵。通常，主体会意识到自己在想象一种有超过两列的点阵。这意味着图像很可能会有意外的排除。在这种情况下，它将排除三行两列的点阵。一个将所有角度都表示为锐角的三角形图像将排除具有钝角或直角的三角形。危险在于，视觉推理对于视觉表示无意中排除的情况可能无效，导致得出结论的步骤是不合理的概括。

我们应该如何看待这个问题？首先，让我们注意到，在一些情况下，图像或图表不会过于具体。例如，在几何学中，所有相关类的实例都是相似的，例如所有圆或所有正方形，图像或图表对于关于该类的概括不会过于具体；因此不会有意外的排除，也不会有不合理的概括的危险。因此，这里有一些可靠的视觉思维证明的可能性。
...
如果 f 是一个定义在闭区间[a,b]上的实变量的实值函数，并且 f(a)<c<f(b)，那么存在一个 x 在(a,b)之间，使得 f(x)=c。
...
* 4. 因此，对于在[a,b]上满足 f(a)<0<f(b)的ε-δ连续函数 f，它具有零值。
...
视觉思维能够在更高级的背景下发现证明思路吗？是的。Carter（2010）给出了一个来自自由概率理论的例子。这个案例涉及到一组自然数的特定排列（在 Carter 2010 中用带有抑扬符号的“p”表示）。使用特定类型的图表，图表的明显属性自然地导致排列的某些属性（交叉和非交叉，具有相邻对），以及某种操作（取消相邻对）。所有这些都有代数定义，但是通过图表思考才能注意到这些定义的思路。对于相关的排列σ，σ(σ(n))=n；因此，一个排列可以用连接点的线集表示。图 8 中左侧和右侧表示的排列分别是非交叉和交叉的，前者具有相邻对{2,3}和{6,7}。
...
推理如下。假设π没有相邻对。选择 j，使得π(j)−j=a 是最小的，也就是说，对于所有的 k，π(j)−j≤π(k)−k。由于π没有相邻对，π(j+1)≠j。所以要么π(j+1)小于 j，我们有一个交叉，要么由于π(j)−j 的最小性，π(j+1)大于 j+a，我们再次有一个交叉。卡特尔报告说，这个分支最初是通过对图表的思考而被认为是正确的，而在发表的论文中给出的引理的证明是这种推理的非图表化“版本”。在这个案例研究中，可视化思维被证明在多个方面对发现有所贡献；特别是，通过引导数学家注意到关键的属性——“定义是基于图表的”——并为整个证明的某些部分提供思路。
...
这里有一些指示。回想一下庞加莱开放圆盘模型的双曲几何：测地线是与边界正交的圆的直径或弧，单位距离表示从中心向边界移动时欧几里得距离越来越短（边界不是模型的一部分）。所有三角形的角和<π（图 13，左），并且存在一个全局常数δ，使得所有三角形在以下意义上都是δ-薄的：

如果且仅如果 T 的一侧上的任意点都在另外两条边的某个点的δ邻域内，那么三角形 T 就是δ-薄的。

这个条件等价于 T 的每条边都在另外两条边的δ邻域的并集内，如图 13 所示。在欧几里得平面中不存在常数δ使得所有三角形都是δ-薄的，因为对于任何δ，都存在足够大的三角形，使得最长边的中点距离其他两条边上的所有点都大于δ。
...
* 如果存在某个δ，使得测地空间的所有三角形都是δ-薄的，则该测地空间是双曲的。
...
> 这个空间（带有字度量的 Cayley 图）对于几何学家来说可能显得乏味和平淡，因为它是离散的，传统的（例如拓扑和无穷小）工具在Γ中无法运行。为了恢复几何视角，人们必须改变自己的位置，并将观察点远离Γ。然后，从距离 d 看到的Γ中的度量变为原始距离除以 d，当 d→∞时，Γ中的点融合成一个连续的实体，占据了视觉地平线，没有任何间隙和空洞，让我们的几何学家心中充满了喜悦。

他在说要将观察点远离Γ，使点融合成一个占据视觉地平线的实体时，明确表示在通往几何群论的道路上，视觉想象力起着至关重要的作用。在从庞加莱圆盘模型思考超几何空间的情况下，视觉思维再次参与了发现超边性作为一般测地线空间的属性。很难想象在没有使用视觉资源的情况下，如何发现这个属性。
...
经过训练使用算盘的人通常通过想象在算盘上移动珠子并按照学习的算术计算程序进行心算（Frank 和 Barner 2012）。这种心算方式被称为心算算盘（MA），在速度和准确性方面与其他种类的心算相比具有优势（Kojima 1954），而且心算算盘的使用者经常在心算世界杯中获得奖牌。
...
视觉经验不仅揭示了论证的步骤，还揭示了它们的有效性，从而为我们接受论证并相信其结论提供了依据。例如，考虑从第二个等式到第三个等式的步骤。除了身份逻辑之外，相关的背景知识是群操作是可结合的。这个事实通常以一个方程的形式表示，它以一种明显的方式重新安排括号：
...
> | ![sep man icon](https://plato.stanford.edu/symbols/sepman-icon.jpg) | [How to cite this entry](https://plato.stanford.edu/cgi-bin/encyclopedia/archinfo.cgi?entry=epistemology-visual-thinking). |
> | --- | --- |
> | ![sep man icon](https://plato.stanford.edu/symbols/sepman-icon.jpg) | [Preview the PDF version of this entry](https://leibniz.stanford.edu/friends/preview/epistemology-visual-thinking/) at the [Friends of the SEP Society](https://leibniz.stanford.edu/friends/). |
> | ![inpho icon](https://plato.stanford.edu/symbols/inpho.png) | [Look up topics and thinkers related to this entry](https://www.inphoproject.org/entity?sep=epistemology-visual-thinking&redirect=True) at the Internet Philosophy Ontology Project (InPhO). |
> | ![phil papers icon](https://plato.stanford.edu/symbols/pp.gif) | [Enhanced bibliography for this entry](http://philpapers.org/sep/epistemology-visual-thinking/) at [PhilPapers](http://philpapers.org/), with links to its database. |

## Other Internet Resources

* Bernazzani, D., 2005, [Soroban Abacus Handbook](https://www.sliderulemuseum.com/Abaci/THE_ABACUS_HANDBOOK.pdf).
* Dalvit, E., 2012, [A Journey through the Mathematical Theory of Braids](http://matematita.science.unitn.it/braids/download.html).
* Fernandes, L. 2015 [The Abacus: A brief history.](https://www.ee.ryerson.ca/~elf/abacus/history.html).
* Lauda, A., 2005, [Frobenius algebras and planar open string topological field theories](https://arxiv.org/abs/math/0508349), at arXiv.org.

## Related Entries

[*a priori* justification and knowledge](https://plato.stanford.edu/entries/apriori/) | [Bolzano, Bernard](https://plato.stanford.edu/entries/bolzano/) | [Dedekind, Richard: contributions to the foundations of mathematics](https://plato.stanford.edu/entries/dedekind-foundations/) | [diagrams](https://plato.stanford.edu/entries/diagrams/) | [mathematical: explanation](https://plato.stanford.edu/entries/mathematics-explanation/) | [proof theory](https://plato.stanford.edu/entries/proof-theory/) | [quantifiers and quantification](https://plato.stanford.edu/entries/quantification/) | [Weyl, Hermann](https://plato.stanford.edu/entries/weyl/)
