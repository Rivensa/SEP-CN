...
> \*\* 经验充分性\*\* _P_ψ _lr_(_x\*\*l_ & _yr_) = ∫λ _P_λ _lr_(_xl_ & _yr_) · ρψ _lr_(λ) _P_ψ _l_(_x\*\*l_) = ∫λ _P_λ _l_(_x\*\*l_ ) · ρψ _l_(λ) _P_ψ _r_(_y\*\*r_) = ∫λ _P_λ _r_(_y\*\*r_) · ρψ _r_(λ).\[[4](https://plato.stanford.edu/entries/qm-action-distance/notes.html#4)]
...
| _P_λ_l r_(_xl_) =_P_λ_l_(_xl_) | and | _P_λ_l r_(_yr_) =_P_λ_r_(_yr_). |
| ------------------------------ | --- | ------------------------------- |

> 结果独立性。EPR/B 实验中远距离测量结果的概率与附近测量结果无关。或者正式地说，对于任意一对的状态λ，L 设置 l，R 设置 r，L 结果 xl 和 R 结果 yr：

| _P_λ_l r_(_xl_/_yr_) =_P_λ_l r_(_xl_) | and | _P_λ_l r_(_yr_/_xl_) =_P_λ_l r_(_yr_) |
| ------------------------------------- | --- | ------------------------------------- |
|                                       |     |                                       |
| _P_λ_l r_(_yr_) > 0                   |     | _P_λ_l r_(_xl_) > 0,                  |
...
> |ψ_8_ >_S_1+_S_2 = ∑_i ci_ |α_i_>_S_1 |β_i_>_S_2.
...
> (PI) _P_λ _lr_(_xl_ & _yr_) + _P_λ _lr_(_xl_ & ¬_yr_) = _P_λ _lr_′ (_xl_ & _y\*\*r_′ ) + _P_λ _lr_′ (_xl_ & ¬_y\*\*r_′ ).
