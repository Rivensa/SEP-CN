...
然后，论证继续如下：在经典统计力学中，粒子的排列变换被视为给出了不同的排列，这意味着虽然它们是无法区分的，但这样的粒子可以被视为个体（事实上，玻尔兹曼在他的《力学讲义》的第一个公理中明确提到了这一点，以不可穿透性假设为基础）。由于这种个体性存在于超越粒子本身的内在属性之上，因此被波斯特（1963 年）称为“超越性个体性”。这个概念可以用各种众所周知的方式来解释，如上面的引言所示：例如，可以用某种基础的洛克式物质来解释，或者用原始的个体性来解释。更一般地说，可以通过模态方式来接近它，通过 haecceitism 的教义：这个教义断言，两个可能的世界可以以相同的方式描述某个个体（即具有相同的属性集），但通过在每个世界中赋予不同的 haecceity 或个体性来以不同的方式表示该个体，或者更一般地，通过将某些非定性的方面归因于个体（Lewis 1986; Huggett 1999a）。
...
上面的粒子和盒子图对应于物理学家的多维“相空间”，描述了个体具有哪些属性，而场论表示对应于“分布空间”，仅描述了以何种数量实例化了哪些属性。Huggett 指出前者支持个体性论，而后者不支持，并且经验证据没有提供选择这两个空间之间的基础（Huggett 1999a）。因此，声称经典统计力学与个体性论相结合的说法也变得可疑。
...
声称同类型的量子粒子在最强的意义上违反了 PII（即它们具有完全相同的关系和非关系属性，无论是状态独立还是状态相关），构成了可以称为正统量子个体性方法的核心。然而，近年来，一种质疑上述不可辨性论的替代观点获得了更广泛的认可。这种观点可以称为异端。它可以追溯到科学实践中我们经常和通常区分属于同一类别的粒子，比如两个电子（Dieks 和 Lubberdink 2011）。在实验室工作的实验者明显区分在云室中检测到的电子和占据仙女座星系特定恒星的不同电子。这些电子在许多物理属性上可能有所不同：它们的时空位置、能量、自旋等。在必然的对称化假设和置换不变性的光线下，这种直觉能否得到坚实的概念和形式基础？事实上，可以。

如何在完全对称（即置换不变）的语言中正式表达可辨别性事实的问题已经在（Ghirardi 等人，2002 年，Ghirardi 和 Marinatto，2003 年）中得到解决。他们使用对称组合的正交单粒子投影算符（投影器）来表达两个（或更多）占据对称或反对称状态的粒子仍然具有不同属性的事实。举个简单的例子：如果 Pup 是代表“自旋向上”（在某个方向上）的投影器，而相应地，Pdown 代表“自旋向下”，那么对称组合 Pup⊗Pdown+Pdown⊗Pup 将表示一个两粒子系统的属性，可以非正式地表达为“一个粒子具有自旋向上，另一个具有自旋向下（但我们不知道哪个是哪个）”。这个想法已经被几位作者（Saunders，2013 年，Caulton，2014a，2014b，Dieks，2020 年，2023 年，Dieks 和 Lubberdink，2020 年，Bigaj，2015c，2022 年）接受并扩展。事实证明（Caulton，2014a），使用上述辨别方法，我们总是可以通过一些正交投影器区分两个相同类型的费米子，但不一定是一维的。与费米子相关的可辨别性陈述可以看作是对泡利排斥原理的一般化。对于相同类型的玻色子，它们的完全不可辨性在相同状态的乘积中仍然存在，例如玻色子凝聚态的情况。因此，根据异端派观点，对于相同的费米子，PII 永远不会被违反，而对于相同的玻色子，其违反情况仅限于特殊状态。

这个事实如何与正式无懈可击且相当直接的论证相一致，这些论证论证了相同类型粒子的完全不可辨性，如（French 和 Redhead 1988，Butterfield 1993 和 Dieks 和 Versteegh 2008）所述？事实证明，所有这些形式推导都预设了 Caulton（2014a）首次确定并由他称为“因子主义”的解释原则。当我们以对称或反对称形式写下两个相同类型粒子系统的状态时，这个状态被认为占据了两个相同希尔伯特空间的张量积：H1⊗H2。因子主义声称这些希尔伯特空间代表构成系统的粒子的状态和属性。因此，我们用来区分总希尔伯特空间分解中的两个组成部分的标签 1 和 2 也可以用来指代个别粒子。事实上，当解释两个粒子的对称/反对称状态时，通常会使用在描述这些状态时使用的标签，并且对称性/反对称性的要求是基于这些标签的。然而，Caulton 观察到，这个假设并不是标准量子力学形式主义强加给我们的。当我们转换到基于对称投影算符的个体化方法时，如上所述，标签不再起到指代的作用。这可以帮助我们理解 Ghirardi 等人（2002）有关对称投影算符使我们能够通过属性区分粒子而不告诉“哪个是哪个”的略带神秘的言论。这可以解释为表达了这样一个事实，即虽然我们确实可以说一个电子自旋向上，另一个电子自旋向下，但我们不能将这些电子与描述它们的联合状态中使用的任何标签相关联，仅仅因为这些标签不再具有指代性。 正如 Caulton 所建议的那样，对称/反对称状态中的标签仅起到一种形式上的作用，作为某种数学对称性的指示器，类似于电磁学或其他物理理论中的规范对称性。
...
异端方法的另一个有趣特点是，它允许粒子在相同的联合态中具有许多不同甚至不兼容的个体化。这个特点被称为任意性（Ghirardi）、约定性（Caulton）或模糊性（Bigaj）。如果我们限制在两个相同费米子的非 GMW 纠缠态的情况下，我们可以证明存在无限多个相互不兼容的单粒子投影算符对，可以个体化这些费米子。这造成了一个概念上的问题，因为我们没有一般的方法来选择一个个体化作为实际现实的代表，然而它们又不能同时都正确。在某些情况下，可以通过实用主义的方式选择一个个体化，例如当我们对具有明确定义的空间位置的个体化感兴趣时，但这种方法无法解决所有可能的情景中的问题。Bigaj（2022）建议我们引入相对存在的概念来解释这一现象，但这个形而上学的提议还需要进一步发展。

异端方法对量子个体性问题的影响是什么？至少在费米子的情况下，我们可以通过属性的绝对可区分性来恢复个体性的理解。但事情并不那么简单。正如我们在上面所看到的，我们必须考虑到量子属性的不确定性这一明显的非经典现象。即使将这个问题放在一边，我们也必须注意到基于跨时空和跨世界认同的个体性条件并不一定能得到满足。事实上，即使在异端方法下，粒子的跨世界（反事实）身份也无法得到确认，除非引入一些重量级的形而上学概念，比如 haecceities 或原始身份。关于跨时空认同，异端方法比其对手正统观点更加灵活。也就是说，只要与这些粒子相关的某些“标记”在相互作用过程中不发生变化，比如某些方向上的自旋（Feynman 等人，1964 年；Cohen-Tannoudji 等人，1978 年；对于散射实验的异议意见和替代处理方法，请参见 Kastner，2023 年；de Barros 和 Holik，2023 年），异端方法允许存在明确的跨时空认同。然而，在典型的情况下，这种认同是不可能的，相互作用的粒子会“融合”它们的身份，从而产生众所周知的干涉效应。因此，即使在异端方法下，量子个体性的概念也变得不稳定。这是否意味着我们应该直接将量子粒子解释为非个体，还是只是修改经典个体性的概念，还有待观察。然而，正统方法的不可辨认性论点无疑更有助于非个体性的概念。 但现在一个明显的问题出现了：我们如何理解这个概念？
...
或者，但与此相关的是，非个体性可以理解为否定自我认同的概念。这个建议在伯恩、薛定谔、赫塞和波斯特的哲学思考中最为突出（伯恩 1943 年；薛定谔 1952 年；赫塞 1963 年；波斯特 1963 年）。然而，这个观点立即就存在明显的问题：我们如何拥有不与自身相同的对象？这种自我认同似乎与对象性的概念紧密相连，因为它是成为该对象的本质部分（因此有人提出，非个体性可能更好地理解为量子物理学中时空轨迹的丧失；参见阿伦哈特、布埃诺和克劳斯 2019 年）。这种直觉可以用奎因的口号“没有实体就没有身份”（奎因 1969 年）来概括，它带来了关于指称等方面的所有相关后果。

然而，巴尔坎·马库斯提出了一种替代观点，坚持“没有实体就没有同一性。”（马库斯，1993 年），并认为尽管“…所有术语都可以“指称”对象…但并非所有对象都是事物，事物至少是适合断言同一性关系的对象。”（同上，第 25 页）。因此，对象引用成为比事物引用更广泛的概念。在这样的框架下，我们可以通过所谓的“薛定谔逻辑”对不具有自我同一性的对象的概念进行形式上的把握，这是由达·科斯塔引入的（达·科斯塔和克劳斯，1994 年）。这些是多分类逻辑，其中表达式 x=y 通常不是一个良构的公式；它适用于 x 和 y 是一种术语的情况，但不适用于对应于量子对象的另一种术语。这样的逻辑可以用“准集合”（达·科斯塔和克劳斯，1997 年）的术语来给出语义。这样发展的动机是量子对象的集合不能像通常的康托尔意义上的集合那样被视为“…我们直觉或思维中明确、不同的对象的整体集合。”（康托尔，1955 年，第 85 页）。准集合理论包括两种基本假设或“原子元素”：m-原子，其预期解释是量子对象，以及 M-原子，代表“日常”对象，属于具有原子元素的经典集合论的范畴。然后，准集合是通过将通常的策梅洛-弗兰克尔框架加上原子元素 ZFU 样的公理应用于由 m-原子、M-原子和它们的聚合物组成的基本域而得到的集合（克劳斯，1992 年；关于准集合理论与准集合理论的比较，参见达拉·基亚拉、吉恩蒂尼和克劳斯，1998 年）。

这些发展为量子“非个体性”提供了一个范畴框架的开端，据称有助于明确这个概念，并且使其在哲学上具有可尊重性（详细内容请参见 French 和 Krause 2006；另请参见 Arenhart 2012；Domenach 和 Holik 2007；Domenach、Holik 和 Krause 2008；Krause 2010）。在这个形式框架中，关键是保留了可数性的意义，即量子实体的集合具有一种（某种程度的）基数，但没有序数性，因此我们实际上可以说有多少个对象，尽管我们无法按数字顺序排列它们。对于这些形式细节以及将“非个体性”归因于量子对象的基础的批评性讨论可以在 Bueno 等人的 2011 年和 Sant'Anna 的 2019 年的著作中找到。这些批评大部分是基于坚持我们不需要采取如此激进的方法的基础上进行的。因此，上面讨论的“弱可辨别性”的支持者认为，这个概念提供了一个适合量子物理学的自然主义个体性的意义。（更具体地说，弱可辨别关系为将数量身份和差异性的事实基于纯粹的定性事实提供了希望，从而削弱了身份不适用于无法区分的量子粒子的论证（但请参见 Bigaj 2015a，对于一个论证，即在量子力学中使用的弱可辨别关系并未实现这一目标）。另一方面，前一节讨论的异端方法恢复了量子粒子的绝对可辨别性，从而消除了非个体形式理论的一个核心论证，但保留了从缺乏再识别标准和唯一的反事实身份的其他论证。另一方面，Dorato 和 Morganti（2013）坚持，正如已经注意到的那样，人们可以保留可数性和个体性作为原始概念，并且这比转向非个体性更可取（有关对后者的回应和对上述形式框架的辩护，请参见 Arenhart 和 Krause 2014）。另一方面，Jantzen 认为，身份和基数是作为“意义问题”而不是形而上学问题相互联系的，因此，谈论没有身份的实体要么毫无意义，要么实际上是在谈论完全不同的事物（Jantzen 2019）。同样，Bueno 坚持认为，身份太基本，不能轻易放弃，并建议我们可以直接从量子粒子的不可区分性中推断出它们的非个体性，身份本身被理解为一种“有用的理想化”，简化了我们的概念框架，并允许我们预测相关对象的行为-在这种情况下是量子实体（Bueno 2014；有关回应，请参见 Arenhart 2017a 和 Krause 和 Arenhart 2019）。

准集合的形式理论已被证明支持一种与非个体粒子不同的属性量子形而上学（Holik 等，2022）。这个概念借鉴了众所周知的物体束理论，由 da Costa、Lombardi 等人发展而来（Lombardi 和 Castagnino，2008；da Costa 等，2013 年；da Costa 和 Lombardi，2014 年；Lombardi 和 Dieks，2016 年；Lombardi，2023a，2023b）。根据他们的提议，基本对象是属性（分为类型属性和案例属性），关于这些属性的基本事实涉及它们的实例化。两个相同粒子的状态可以描述为同一属性的多个实例化的情况（这种情况被称为“聚合”或“合并”束）。由于束不是个体，不适用于它们的是不可辨识性原则。此外，这种方法不涉及重新识别（历时身份）和反事实身份的问题。然而，属性的本体论可能被指责与科学实践相悖，因为在各种实验环境中，粒子的概念仍然被使用（参见 Dieks 2023 年提出的使用粒子概念作为新兴实体的建议）。
...
在这些极端之间，有各种处理不确定性的选项，对应着不同层次的“深度”现实主义。因此，人们可能会尝试辩称不确定性可以以某种方式“打破”。例如，可以诉诸某种形而上学因素来支持一个方案而非另一个，或者转向元-形而上学的考虑，以辩称基于弱可辨别性的个体性在某些方面优于竞争方案和非个体性，以及其伴随的非标准形式基础。然而，阿伦哈特认为，弱可辨别性会产生进一步的形而上学不确定性，因此不能像一些支持者所声称的那样，支持量子力学的完全自然主义理解（阿伦哈特 2017b，另见阿伦哈特 2023，在其中他对基于量子力学等一般理论解决个体性的形而上学问题的可能性表示怀疑）。当然，也可以反过来辩称非个体性方案避免了在不同形而上学个体性解释之间做出选择，并且准集合论的形式转变并不像想象的那么戏剧性。然而，最终，各种因素的重要性并不明确，甚至是否可以首先应用一个连贯的权重方案也不清楚。

相反，人们可以诉诸广泛的方法论因素来打破不确定性。因此，有人认为，将物体-非个体的组合与量子场论（QFT）更好地结合在一起，据称，从一开始就避免了对个体的讨论（Post 1963；Redhead and Teller 1991 and 1992；Teller 1995）。这一主张的核心论据集中在这样一个核心理解上，即在量子物理学中，物体确实可以被视为个体，但作为个体，它们受到所能占据的状态集的限制。对于某种特定类型的粒子，例如电子，无法访问的状态可以被视为“多余结构”。特别是，如果将粒子视为个体的观点被采纳，那么为什么这些无法访问的多余状态中的一个特定子集，即非对称状态，实际上没有实现，这完全是个谜。应用一般的方法论原则，即不包含这种多余结构的理论优于包含这种结构的理论，Redhead 和 Teller 得出结论，我们有理由更喜欢非个体的组合，并且无法访问的状态的谜团根本不存在（Redhead 和 Teller 1991 和 1992）。

这种论证方式受到了 Huggett 的批评，理由是表面上的神秘只是一种虚构：无法访问的非对称状态可以被排除，因为它们在物理上根本不可能存在（Huggett 1995）。因此，多余的结构是所选择的表示方式的结果，没有进一步的形而上学意义。然而，有人坚持认为理论还应告诉我们为什么某种情况不可能发生。因此，考虑一下可能发生的情况，即一杯冷茶突然开始沸腾。统计力学可以解释为什么我们从未观察到这种可能性，而量子物体作为个体的观点无法解释为什么我们从未观察到非对称状态，因此在这方面存在缺陷（Teller 1998）。

不幸的是，这个类比是有问题的。统计力学并不是说上述情况从不发生，而是说其发生的概率极低。那么问题就变成了“为什么这个概率如此低？”对此的答案通常是以茶水沸腾对应的状态数量非常少，而保持冷的状态数量非常多来解释。那么，为什么可访问状态的数量存在这种差异呢？或者换句话说，为什么我们发现自己处于熵增的情况中？一个答案将我们带回到宇宙大爆炸的初始条件。在量子统计的情况下，也可以采取类似的观点。为什么我们从未观察到非对称状态？因为宇宙就是这样的，我们不应该期望量子力学单独解释为什么某些初始条件得到而其他条件没有得到。在这里，我们要记住哈密顿量的对称性确保了如果一个粒子一开始处于特定对称性的状态（比如玻色-爱因斯坦统计或费米-狄拉克统计），它将一直保持在具有该对称性的状态中。因此，如果非对称状态在宇宙初始条件中不存在，那么它们将永远无法被粒子访问到。因此，问题转变为对上述“多余结构”的意义的不同观点（参见 Belousek 2000）。

此外，即使我们接受“结构越少越好”的方法论原则，也不清楚以非个体“量子”来理解量子场论是否在这方面提供了任何重要优势（尽管参见 da Costa 和 Holik 2015 年关于具有未定义粒子数的态的解释）。实际上，有人认为量子场论的形式主义也与将对象视为个体的替代方案相容。Van Fraassen 提出了这一观点（1991 年），他引用了 de Muynck 关于涉及标记粒子的量子场论状态空间的构建（1975 年）。然而，Butterfield 认为量子场论中存在的粒子数叠加态削弱了这种等价性（1993 年）。然而，Huggett 坚持认为，在这种情况下，削弱是经验性的，而不是方法论的（Huggett 1995 年）。当粒子数是恒定的时候，任意粒子数的态就是多余的结构，现在，如果应用方法论的论证，个体的方案更值得选择。
...
进一步处理这种不确定性的方法是拒绝这两种观点，寻求第三种方式。因此，莫尔甘蒂认为，上述两种形而上学观点都假设关于对象的所有定性特征都必须以其所具有的属性来编码（莫尔甘蒂，2009 年）。放弃这种假设使我们能够将量子统计视为描述整个组装体的“固有”属性。相关状态的（反）对称性则通过系统对测量产生特定相关结果的倾向来解释。这被提出为特勒的“关系整体主义”（特勒，1989 年）的延伸，与此相关的，“固有性”的概念涉及整体属性不随部分属性而随之的否定。然而，正如刚才指出的，这也有一个代价：即承认整体的整体性倾向属性和这些属性在量子背景下的形而上学需要进一步发展，以及系统相互作用时这些固有属性“出现”的方式。早些时候，拉文提出了类似的形而上学观点，认为量子物体可以被视为最小可能的“物质”量，而且关键是，多粒子状态代表了进一步的物质量，因此不包含适当的部分（1991 年；另见詹岑，2019 年）。他声称，这种观点避免了个体和非个体观点中形而上学上的问题。当然，然后还有关于“物质”的形而上学和逻辑的问题，但可以认为这些问题是熟悉的，不仅仅是量子力学的特有问题。其中一个问题涉及“物质”的本质：它是我们熟悉的原始物质吗？作为一种基本的形而上学原理，物质面临着众所周知的困难，有人建议应该放弃它，而采用某种形式的“束理论”，正如本文开头提到的。 如果将个体对象理解为“特质”的束缚，其中特质是属性或关系的个体实例，如果这个概念被扩大以包括其存在依赖于不是它们的一部分的其他个体，那么据称这个概念可能足够灵活以适应量子物理学（Simons 1998; 另见 Morganti 2013）。另一个问题涉及“物质”如何组合：我们如何从由两个独立光子表示的物质量，转变为由一个联合的双光子态表示的物质量？Lavine 给出的类比是众所周知的：水滴，银行里的钱，绳子上的凸起（Teller 1983; Hesse 1963）。当然，这些类比也可以被非个体对象的观点所采用，但更重要的是，它们暗示了一种场论方法，其中所讨论的“物质”是量子场。

在这里，我们回到了关于量子场论的形而上学问题，并值得指出，在这里也可能出现不确定性。在经典物理学中，我们面临着一个选择，即将场视为一种全局物质或物质的观点，或者用场量的概念来描述，这些场量被分配给并因此成为时空点的属性。在量子场论的情况下，由于在量子场论中定义精确的位置状态存在困难，场量在这些点上并没有明确定义，而是被视为在时空区域上“扩展”（参见 Teller 1999）。当然，不确定性仍然存在：在将给定的量子场理解为某种全局物质的观点和将其理解为时空区域的属性之间。显然，选择第一种选项需要一种适用于量子场论的形而上学实体论形式。许多评论家更喜欢第二种选择，但现在，当然，必须注意到场属性被认为是实例化的时空区域的形而上学地位。通常，这些被认为是由时空点组成的，并且将场构想为一组属性与将时空视为一种物质或“物质”本身的方法相吻合。但是，在现代物理学的背景下，这也面临着众所周知的困难（例如，参见 Earman 1989）。特别是，时空实体论被认为具有极其令人难以接受的后果（Earman 和 Norton 1987）。 不幸的是，基于属性的场的解释很难与将时空视为仅仅是物体之间关系（如接触）的一种观点相协调：如果场量是时空区域的属性，而后者最终被理解为可归约为物体之间的关系，而后者又以场论术语来构思，那么就会出现循环性（参见 Rovelli 1999）。前进的一种方式是借鉴关于时空本质的替代解释。因此，Stachel 提出我们放弃对物体和物体之间关系之间的明确的形而上学区分，采取广义的“结构主义”视角来看待时空（Stachel 1999；参见 Rickles、French 和 Saatsi 2006 中的文章）。适当扩展，这种“结构主义”方法可能通过将时空和量子场都视为结构性的术语，而不是物质、属性或关系的术语，来解决上述的不兼容性（参见 Auyang 1995；Cao 2003；French 和 Ladyman 2003；Kantorovich 2003；Lyre 2004；Saunders 2003b）。

这使我们进一步思考了对上述形而上学不确定性的可能回应，这促使现实主义从物体的形而上学撤退，并发展与物理学相容的结构本体论（Ladyman 1998 和 2014）。在量子背景下，早期尝试可以在卡西勒的工作中看到，他注意到新物理学对个体物体的标准概念的影响，并得出结论，量子物体只能被描述为某些关系的“交点”（1937 年，第 180 页）。抛开卡西勒结构主义中的新康德主义元素，这种对量子实体的看法已经在“实体”结构现实主义的背景下得到发展（Ladyman 和 Ross 2007）。借鉴 Weyl 和 Wigner 的观点，量子物体在这里被理解为本体论上构成的，以群论的方式，通过不变量集合来描述，例如静质量、电荷、自旋等（Castellani 1998a）。从这个角度来看，个体性和非个体性的观点都是错误的，因为它们假设根据物理学的观点，世界的本质是一个由物体构成的世界，这些物体可以被视为个体，无论是原始地还是通过弱可辨别性，或者可以被视为非个体，无论是通过准集合论还是其他方式进行形式化表示。那么，我们应该如何看待我们在量子背景下讨论同一性和个体性时开始的“不可区分性原则”？上述观点都基于对粒子置换的某种理解，正如该原则所概括的那样，即这些置换应该被理解为在状态之间交换粒子，或者在我们的示意图中交换盒子。然而，我们也可以将“不可区分性原则”视为对量子力学的一种基本对称性约束，即相关状态应该在粒子置换下保持不变。 一种替代的观点是，将“置换不变性”视为与对称原理的普遍接受观点相一致，它表达了形式主义中的某种表征冗余。因此，根据我们上面的草图，每个盒子中一个粒子的置换排列，在经典统计力学中计算，但在量子形式中不计算，可以在这个意义上被认为是“表征上的冗余”。这将“置换不变性”视为现代物理学中具有基本作用的一种对称原理（Huggett 1999b; French and Rickles 2003）。或许并不令人意外的是，这种重新解释可能也在形而上学上产生影响，因为当应用于遵循费米-狄拉克统计的某些“物质”粒子系统时，这些系统的组成（从可以被视为由“部分”组成的子系统的角度来看）违反了标准的部分整体原则（对这个问题的异端观点的分析可以在 Caulton 2015 中找到；关于一些可能的回应，请参见 Bigaj 2016）。更一般地，有人认为“置换不变性”与粒子本体论甚至在形而上学上的最小意义上的理解是不兼容的（Jantzen 2011）。鉴于前者的基本意义，有人建议必须放弃后者。一个可能的替代方案是采用一种时空实体主义形式，并将具有属性的时空区域作为适当的本体基础（Jantzen 2011）。然而，这会遇到上面提到的问题。 更激进地说，也许放弃上述的“以对象为导向”的假设将完全削弱形而上学的不确定性，并为另一种本体论提供空间，其中量子实体被看作是“世界结构”的特征之一（参见 French 和 Ladyman 2003）。这可以通过相关的定律和对称性来表达，将这些假设实体的属性理解为这种结构的确定性方面（参见 French 2014；关于“结构实在论”背景下这种本体论的进一步考虑，参见 Ladyman 2014；关于一些反对意见，参见 Sider 2022，第 3.4 节）。
