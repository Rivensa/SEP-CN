...
竞争的数学策略在关于量子力学数学基础的最近讨论中得以体现。在冯·诺依曼（1955）关于该主题的论著的前言中，他指出狄拉克为量子力学提供了一个非常优雅和强大的形式框架，但对该框架中一个“具有自相矛盾属性的不适当函数”在其中的核心地位表示不满，他还将其描述为一个“数学虚构”。他指的是狄拉克的δ函数，该函数具有以下不兼容的属性：它在实数线上定义，除了一个点处为无穷大外，其他地方都为零，并且在实数线上积分后得到单位。冯·诺依曼提出了一种替代框架，他将其描述为“同样清晰和统一，但没有数学上的异议”。他强调他的框架不仅仅是对狄拉克的改进，而是一种基于希尔伯特算子理论的根本不同的框架。

当然，狄拉克完全意识到δ函数不是一个明确定义的表达式。但他对此并不感到困扰，原因有两个。首先，只要遵循δ函数的规则（例如只在积分符号下使用δ函数，部分意味着不要求δ函数在给定点的值），就不会出现矛盾。其次，δ函数可以被消除，也就是说可以用一个明确定义的数学表达式来替代它。然而，狄拉克认为，在这种情况下的替代会导致一个更加繁琐的表达式，使论证变得模糊不清。简而言之，当实用主义和严谨性得出相同的结论时，由于结果的简单性、效率性和理解度的提高，实用主义胜过严谨性。

与无穷小概念一样，狄拉克δ函数最终得到了严格的数学基础。这是在施瓦茨的分布理论中完成的，后来被用于发展装配希尔伯特空间的概念。分布理论被用于为量子场论提供数学框架（Wightman 1964）。装配希尔伯特空间被用于为量子力学（Böhm 1966）以及量子场论（Bogoluliubov等人，1975）提供数学框架。

在量子力学的发展中，互补的方法、严谨性和实用主义在量子电动力学（QED）以及更一般的量子场论（QFT）的发展中以更引人注目的方式出现。严谨性的重点体现在两个框架中，即代数量子场论和Wightman的公理量子场论。代数量子场论的根源可以追溯到冯·诺依曼（von Neumann）关于算子代数的工作，他试图将希尔伯特空间框架推广。Wightman的公理量子场论的根源可以追溯到施瓦茨（Schwartz）的分布理论，并且后来在装配希尔伯特空间框架中得到发展。大致上，这两种方法之间的基本区别在于，算子代数是代数量子场论中的基本数学概念，而算子值分布（场量子的量子类比）在Wightman的公理量子场论中是基本的。值得注意的是，代数量子场论通常是公理化表述的，而且它同样值得被称为“公理化”量子场论。然而，这个术语通常被理解为特指基于算子值分布的方法。为了避免任何可能的混淆，在这里将这种方法称为“Wightman的公理化”量子场论。实用主义的重点主要体现在拉格朗日量子场论中，它使用微扰论、路径积分和重整化技术。尽管该理论的一些要素最终被放在了更坚实的数学基础上，但仍然存在一些严重的问题，即它是否是与代数量子场论和Wightman的公理量子场论相媲美的完全严谨的方法。然而，它在提供与实验测定量相当准确的数值结果方面取得了巨大成功，并且使得其他方法无法比拟的便捷计算成为可能。
...
在20世纪20年代末，冯·诺伊曼发展了可分离的希尔伯特空间量子力学表述，后来成为了（至少从数学严谨性的角度来看）确定的表述。在20世纪30年代中期，他广泛研究了格理论（参见量子逻辑条目）、算子环和连续几何。他发展这些数学理论的部分动机是为了为量子场论提供适当的框架和量子力学的更好基础。在此期间，他注意到了两个密切相关的结构，模块格和有限的II型因子（一种特殊类型的算子环），他认为这些结构对于量子理论具有理想的特征。这些观察结果导致他发展了一个更一般的框架，连续几何，用于量子理论。然而，冯·诺伊曼很快意识到，如果连续几何要用于描述量子力学现象，它们必须具有转移概率函数，并且所得到的结构根本不是超出已有的算子环的一般化。此外，后来发现，III型因子是量子理论中最重要的算子环类型。此外，关于格理论，后来也得出了类似的结论。适用于量子理论的格只有在是模块的情况下才是正交模块的，但反之不成立。在这三个数学理论中，算子环被证明是量子理论中最重要的框架。可以在纯抽象的代数设置中使用算子环来模拟物理系统的关键特征（这在第4.1节中讨论）。 一个相关问题是是否有必要在希尔伯特空间中选择环的表示；有关这个问题的进一步讨论，请参阅 Haag 和 Kastler（1964），Ruetsche（2003）和 Kronz 和 Lupher（2005）。无论如何，可分离的希尔伯特空间仍然是量子理论的关键框架。可分离的希尔伯特空间的最简单的例子是有限维的，此时算符代数是一种类型-In 因子（n 是正整数）。这些算符是 n 乘 n 的复矩阵，通常用于描述内部自由度，如自旋。想要熟悉这些基本例子的读者应该参阅有关量子力学的条目。
...
薛定谔是第一个提出矩阵力学和波动力学之间关系问题的人，他在1926年的《Annalen》杂志上发表了一篇名为《薛定谔》的论文，该论文发表在他四篇著名论文中的第二篇和第三篇之间。这篇论文也被收录在薛定谔（1928）的著作中。它包含了一个数学等价证明的种子，但并没有包含严格的等价证明：薛定谔将与波动力学相关联的数学框架视为连续且可归一化函数的空间，这个空间太小，无法与矩阵力学建立适当的关系。不久之后，狄拉克和约旦分别提出了两个框架的统一。但他们各自的方法都需要基本使用δ函数，从数学严谨性的角度来看，这是可疑的。1927年，冯·诺伊曼在《Göttinger Nachrichten》上发表了三篇论文，为量子力学奠定了严格的数学基础，并包含了一个严格的证明（即不使用δ函数）矩阵力学和波动力学的等价性。这些论文被收录在冯·诺伊曼（1961-1963，第一卷，第8-10期）的著作中。在他著名的1932年量子力学论文（冯·诺伊曼1955）的前言中，他总结了他在早期论文中提供的可分离希尔伯特空间量子力学的优雅概述，他承认狄拉克的量子力学公式的简洁和实用性，但最终认为它是不可接受的。他指出，他无法忍受当时只能被视为数学虚构的使用。这些虚构包括狄拉克假设每个自伴算符都可以被对角化以及他使用的δ函数，冯·诺伊曼将其描述为“具有自相矛盾属性的不适当函数”。他的目标是建立一个数学严谨的量子力学框架。
...
在1935年写给Birkhoff的一封信中，冯·诺伊曼说：“我想要做一个可能看起来不道德的自白：我不再相信希尔伯特空间了”；这封信发表在冯·诺伊曼（2005）的著作中。这个自白确实令人震惊，因为它来自量子力学可分离希尔伯特空间表述的倡导者，而且正是在他出版了关于这个主题的权威著作三年后发表的。讽刺的是，在他向Birkhoff坦白的不到两年后，他关于将取代可分离希尔伯特空间的抽象数学结构的数学理论化，即具有过渡概率的连续几何，事实证明并没有提供可分离希尔伯特空间框架的推广。更有趣的是，随后由冯·诺伊曼引发和发展的数学物理学的进一步发展最终加强了可分离希尔伯特空间框架在数学物理学中的根深蒂固（尤其是关于量子理论）。这些问题在第4.1节中有更详细的解释。

冯·诺伊曼在1932年之后的七年间，将三个理论发展融合到他的连续几何理论中：代数方法的量子力学、量子逻辑和算子环。到1934年，冯·诺伊曼已经在量子力学的代数方法方面取得了重要进展，得到了乔丹和维格纳的帮助——他们的文章《量子力学形式主义的代数推广》被收录在冯·诺伊曼的著作中（1961-1963年，第二卷，第21号）。1936年，他发表了关于这个主题的第二篇论文《量子力学形式主义的代数推广（第一部分）》，被收录在冯·诺伊曼的著作中（1961-1963年，第三卷，第9号）。但事实证明，这两篇论文并没有特别大的影响力。冯·诺伊曼和伯克霍夫合著的一篇相关论文《量子力学的逻辑》也于1936年发表，并被收录在冯·诺伊曼的著作中（1961-1963年，第四卷，第7号）。这篇论文对量子逻辑的发展具有开创性意义。然而需要注意的是，这仅在冯·诺伊曼的关键假设之一——模性（modularity）被替换为正交模性（orthomodularity）之后才发生。这种转变的性质在霍兰德（1970）中得到了清晰的解释：模性实际上是对分配律的削弱（将其有效性限制在某些选定的格元素三元组上），而正交模性是对模性的进一步削弱（将分配律的有效性限制在更小的格元素三元组集合上）。从模性到正交模性的转变首次出现在卢米斯（1955）的工作中。随后，关于正交模格和量子力学基础的文献迅速增长。例如，可以参考帕维奇奇（1992）对截至1990年的量子逻辑的相当详尽的参考文献，其中包含1800多个条目。

对于量子理论的基础来说，冯·诺伊曼的六篇论文更为重要。其中三篇与默里合作，讨论了算子环，这些论文被收录在冯·诺伊曼（1961-1963，第三卷，第2-7号）中。前两篇论文《关于算子环》和续篇《关于算子环II》分别发表于1936年和1937年，对其他四篇的发展具有开创性的影响。第三篇《关于算子环：约化理论》写于1937-1938年，但直到1949年才发表。第四篇《关于无限直积》发表于1938年。剩下的两篇《关于算子环III》和《关于算子环IV》分别发表于1941年和1943年。这部关于算子环的大作对纯数学、数学物理和物理基础学科产生了很大影响，并且至今仍然具有重要意义。现在，算子环被称为“冯·诺伊曼代数”，这个名称是迪克斯米尔（1981年）在他1957年关于算子代数的专著引言中首次提到的（迪克斯米尔，1981年），他表示这个名称是根据迪厄多内的建议而命名的。

von Neumann代数是Hilbert空间H上有界算子B(H)的∗-子代数，它在弱算子拓扑下是闭合的。通常假设von Neumann代数包含单位算子。∗-子代数包含代数中每个算子的伴随算子，其中“∗”表示伴随。有一种特殊类型的von Neumann代数被称为“因子”。如果von Neumann代数的中心（即与代数中所有元素都可交换的元素集合）是平凡的，即只包含标量倍数的单位元素，则该von Neumann代数是一个因子。此外，von Neumann在他的约化理论论文中证明了所有不是因子的von Neumann代数都可以分解为因子的直和（或积分）。有三种互斥且穷尽的因子类型：I型、II型和III型。每种类型都被分类为（互斥且穷尽的）子类型：I\_n型（n=1,2,…,∞）、II\_n型（n=1,∞）、III\_z型（0≤z≤1）。如上所述，I\_n型对应有限维Hilbert空间，而I∞型对应提供波动和矩阵力学严格框架的无限维可分离Hilbert空间。von Neumann和Murray区分了I型和II型的子类型，但对于III型因子无法做到这一点。直到20世纪60年代和70年代，才对这些因子进行了子类型的区分-详见Sunder（1987）的第3章或Connes（1994）的第5章。

由于他在量子力学基础和与Birkhoff合作的量子逻辑方面的早期工作，冯·诺伊曼开始认为II1型因子可能是物理学中最相关的。这是一个重大的转变，因为当时量子力学中最重要的可观测代数类被认为是无限维可分希尔伯特空间上的有界算子集合，即I∞型因子。下面简要解释了这种转变。有关冯·诺伊曼在量子逻辑、算子环（特别是II1型因子）、概率论基础和量子物理之间的基本联系的观点，可以参考（Rédei 1998）中提供的知识渊博且清晰的解释。值得注意的是，冯·诺伊曼将III型因子视为“病态”算子代数的综合类别；事实上，在分类方案引入后的几年里，才证明了这种因子的存在。讽刺的是，现在普遍认为III型因子是物理学中最相关的类别（特别是对于量子场论和量子统计力学）。在解释为什么冯·诺伊曼的计划从未实现之后，本文将在4.1节进一步阐述这一观点。

在题为《算子环》系列四篇论文的第一篇引言中，Murray和von Neumann列举了两个原因，说明他们对可分离的希尔伯特空间量子力学的表述感到不满意。一个原因与迹运算的性质有关，迹运算出现在测量结果的概率定义中（即波恩规则），另一个原因与无界可观测算子引起的定义域问题有关。当可分离的希尔伯特空间是无穷维时，单位算子的迹是无穷大的，这意味着不可能为实验结果（即可观测量的测量）定义一个正确归一化的先验概率。根据定义，实验的先验概率是任意两个不同结果等可能发生的概率。因此，当存在无穷多个这样的结果时，即空间是无穷维的时候，每个不同结果的概率必须为零。不清楚为什么von Neumann认为每个实验都需要有一个先验概率，尤其是因为von Mises明确认为先验概率（他的术语中的“均匀分布”）并不总是存在（von Mises 1981，第68页以下），而von Neumann在概率基础上受到von Mises的重大影响（von Neumann 1955，第198页脚注）。后来，von Neumann将他对无穷维希尔伯特空间不满的表达原因从概率性考虑改为代数性考虑（Birkhoff和von Neumann 1936，第118页）；即违反了Hankel的形式法则保持原则，这导致人们试图保持模块性——这是有限维希尔伯特空间中成立的条件，但在无穷维希尔伯特空间中不成立。 无界算子的问题在于它们仅在空间的一个稠密子集上定义。这意味着无界算子的代数运算（求和和乘积）不能被普遍定义；例如，可能存在两个无界算子 A、B，使得 B 的值域和 A 的定义域不相交，在这种情况下，乘积 AB 是没有意义的。
...
### 3.1 狄拉克的δ函数、原理和布拉-凯特符号

狄拉克试图证明矩阵力学和波动力学的等价性，关键使用了上述的δ函数。在狄拉克之前，物理学家们已经使用过δ函数，但直到狄拉克在量子力学中成功地运用它之后，它才成为物理学许多领域中的标准工具。狄拉克的教材（狄拉克 1930 年）基于他在剑桥大学讲授的一系列量子力学课程而广为人知。该教材后来出版了三个版本：1935 年的第二版，1947 年的第三版和 1958 年的第四版。第四版已经多次重印。它的持久力部分归功于狄拉克在第三版中引入的另一项创新，即他的 bra-ket 符号体系。他首次在（狄拉克 1939 年）中发表了这个符号体系，但直到他的书的第三版出版后，这个符号体系才得到广泛使用。毫无疑问，这些工具，首先是δ函数，然后是 bra-ket 符号，对于从事和教授量子力学的物理学家来说，在建立方程和进行计算方面都非常有效。大多数量子力学教材使用δ函数和平面波，这是狄拉克形式框架的关键要素，但它们并不包含在冯·诺依曼对量子力学的严格数学框架中。从事量子力学研究的物理学家以及量子力学的教师和学生经常使用狄拉克的框架，因为它简单、优雅、强大且相对容易使用。因此，从实用主义的角度来看，狄拉克的框架比冯·诺依曼的框架更受欢迎。装配希尔伯特空间的概念为狄拉克的框架奠定了坚实的数学基础。
...
Baker（2016）和 Swanson（2017）指出，LQFT 做出了错误或未经证明的假设，例如微扰理论中某些无穷级数的收敛性。 Dyson（1952）给出了一个启发性的论证，量子电动力学微扰级数不收敛。 Baker 和 Swanson 还认为，使用长距离截断与宇宙学理论和天文观测相矛盾，后者表明宇宙是空间无限的。即使在弱耦合极限下，微扰理论可以形式上应用，但并不清楚微扰量子场论何时给出底层物理的准确近似。在相互作用ϕ4 理论中，当闵可夫斯基时空的维数小于 4 时，该理论是非平凡的，但当维数大于 4 时，重整化微扰级数渐近于自由场理论，尽管它似乎描述了非平凡的相互作用。当存在 4 个维度时，如果满足额外的技术假设，该理论也是平凡的（有关更多详细信息，请参见 Swanson 2017（第 3 页））。

#### 5.1.2 量子场论中的路径积分

在微扰量子场论中，数学严谨性的问题还存在于路径积分的使用中。S-矩阵的幂级数展开包含动量空间上的积分，而路径积分/费曼图在计算中非常有帮助。关键概念是分区函数Z，它被定义为涉及作用量的泛函积分，而作用量本身是拉格朗日量的积分。以下细节主要来自Hancox-Li（2017）。更具体地说，作用量是量子场的泛函。对作用量的泛函积分涵盖了时空中量子场值的所有可能组合。非正式地说，这个求和是在所有可能的场配置上进行的。正如Swanson（2017）所指出的，路径积分需要在无限维路径空间上选择一个度量，这只在特殊情况下在数学上是明确定义的。例如，如果系统在一个超立方格上被规定，那么度量可以被定义（参见James Fraser 2016的第1.2节）。另一种获得明确定义度量的方法是将注意力限制在有限维子空间上。但是，如果函数在短长度尺度上被允许任意变化，那么积分就不再是明确定义的（Wallace 2006，第42页）。所有的相关函数（即，在不同时空点上的场的真空态期望值）都可以从分区函数Z中推导出来。因此，给定Z，可以计算与拉格朗日量相关的所有经验量，例如散射截面。找到Z相当于解决了LQFT。Z可以用耦合常数的泰勒级数展开。在这样做时，可能会出现两种类型的发散：（1）微扰级数的各个项可能发散，和/或（2）微扰级数本身发散，尽管级数可能是渐近级数。为了处理（1），物理学家进行以下步骤（Hancox-Li 2017，第pp.） 344–345): (i) 正规化，通过维度正规化、动量截断或使用格点形式来减少自由度的数量，以及 (ii) 添加抵消 (i) 中正规化的对消项。但这种构造纯粹是形式上的，没有数学定义。用于操作拉格朗日量和分区函数的规则并不明确。

#### 5.1.3 重整化群技术

Wallace（2011）认为，重整化群技术已经克服了旧的重整化计算技术的数学缺陷（有关重整化群的更多细节，请参见Butterfield和Bouatta 2015，Fraser 2016，Hancox-Li（2015a，2015b，2017））。根据Wallace的观点，重整化群方法使得LQFT在数学严谨性方面与其他理论物理领域处于同一水平。它提供了一个坚实的理论框架，对粒子物理学和凝聚态物理学具有丰富的解释性，因此公理化QFT的动力已经解决。重整化群技术预设了QFT在某个短长度尺度上会失败，但是LQFT的经验内容在这些短长度尺度的细节上基本不敏感。Doreen Fraser（2011）认为，重整化群方法有助于阐明QFT的经验内容，但是重整化群对于QFT的理论内容没有意义，因为它不能告诉我们是否应该关注LQFT还是AQFT。James Fraser（2016）和Hancox-Li（2015b）认为，重整化群不仅在QFT中提供经验预测，还为研究不同能量尺度下物理系统的行为提供了方法，即QFT模型的性质如何依赖或不依赖于小尺度结构。重整化群为微扰QFT的成功提供了非微扰解释。Hancox-Li（2015b）讨论了在建设性QFT中工作的数学家如何使用具有良好控制误差界限的非微扰近似来证明紫外固定点的存在或不存在。Hancox-Li认为，重整化群以非微扰方式解释微扰重整化。重整化群可以告诉我们某些拉格朗日量是否具有满足QFT应满足的公理的紫外极限。 因此，在建设性量子场论中使用重整化群可以提供额外的动力学信息（例如，某种动力学是否可以在连续时空中发生），而纯公理方法则无法提供。
...
构造性量子场论是试图通过严格构建特定的相互作用量子场论模型来调和局域量子场论（LQFT）和公理量子场论（axiomatic QFT）之间关系的一种尝试。它构建的非平凡解应该对应于粒子物理学家使用的拉格朗日量。这确保了各种公理系统通过LQFT的实证成功与世界有物理联系。虽然构造性量子场论已经在某些维度小于4的模型中实现了这一点，但尚未在粒子物理学家使用的4维拉格朗日量中实现。满足奥斯特瓦尔德-施拉德公理的任何模型都会自动满足韦特曼公理。构造性量子场论试图通过从闵可夫斯基时空转向欧几里得时空来构建路径积分的泛函积分测度，这是通过维克旋转实现的（以下内容基于Hancox-Li（2017）的第四节）。在欧几里得场论中，以Z为定义的施温格函数必须满足奥斯特瓦尔德-施拉德公理。Z的测度是对快速衰减函数的Schwartz空间的高斯测度。奥斯特瓦尔德-施拉德公理与韦特曼公理相关，根据奥斯特瓦尔德-施拉德重构定理，任何满足奥斯特瓦尔德-施拉德公理的函数集确定了一个唯一的韦特曼模型，其施温格函数形成该集合。它使得构造性场论学家能够利用欧几里得空间的优势来定义测度，同时确保他们构建的模型存在于闵可夫斯基时空中。仍然需要验证解是否对应于粒子物理学家在LQFT中推导出的相应拉格朗日量的重整化微扰级数。挑战在于如何将数学上不明确的东西转化为数学上明确的东西，并展示LQFT中的“解”可以通过与一组公理一致的东西来复现。 这是至关重要的，因为正如Swanson（2017）指出的那样，摄动理论是否是描述LQFT所描述的基础物理的准确指南尚不清楚。这导致Hancox-Li（2017）认为，在建立严格的相互作用QFT模型的严格计划中，数学上不严谨的LQFT与之相关。这些模型对应于粒子物理学家感兴趣的拉格朗日量。因此，LQFT可以为QFT的理论内容提供信息。

构造性量子场论的另一个工具是渐近级数的使用，它可以告诉我们微扰级数渐近于哪个函数，而微扰量子场论则不能。构造性量子场论试图确定非微扰解的某些性质，以确保某些求和渐近展开方法将导致唯一解（有关更多详细信息，请参见Hancox-Li 2017年，第349-350页）。严格定义的配分函数Z是否渐近于重整化微扰级数？粗略地说，当级数的连续项提供了对函数增长速度的越来越准确的描述时，函数渐近于级数展开。函数与每个微扰级数的差异近似较小。但是，有许多不同的函数具有相同的渐近展开。理想情况下，我们希望存在一个唯一的函数，因为这样就有了一个唯一的非微扰解。强渐近性的概念要求函数与级数的每个阶数之间的差异小于渐近性所要求的差异。强渐近级数唯一确定一个函数。如果存在一个强渐近级数，那么可以通过Borel求和从级数中唯一重构函数。级数的Borel变换通过将级数中每个项的系数除以该项的阶数的阶乘，然后进行积分以恢复精确函数。在构造性量子场论中，目标是将一个唯一的函数与重整化微扰级数关联起来，而某种形式的Borel求和是迄今为止的主要候选方法，尽管Borel变换无法消除高阶发散。 重整化摄动级数的渐近行为对正规化的选择非常敏感，即使它看起来描述了非平凡的摄动，也可能使其渐近于自由场理论（详见Swanson 2017年第11页）。
