...
对于类型 I_n（n<∞）和类型 II1，上述问题并不会出现。也就是说，这些因子类型具有有限的迹运算，并且不会受到无界算子的定义域问题的困扰。特别值得注意的是，每种因子类型（类型 I_n，其中 n<∞，和类型 II1）的投影格是模态的。相比之下，无限维可分希尔伯特空间上的有界算子集，即类型 I∞因子，不是模态的；而是正交模态的。这些考虑说明了为什么冯·诺伊曼将类型 II1 因子视为量子物理学中类型 I_n（n<∞）的适当推广，而不是类型 I∞因子。上述文献中从模态格到正交模态格的转变实际上是回到了冯·诺伊曼早期的立场（在他的自述之前）。但是，正如前面提到的，现在看来这也不是最好的选择。
