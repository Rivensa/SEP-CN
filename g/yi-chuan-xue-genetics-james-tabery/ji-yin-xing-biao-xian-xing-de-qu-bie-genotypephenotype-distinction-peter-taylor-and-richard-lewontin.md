...
> 试图构想或“解释”假定从父母或更遥远祖先“继承”而来的一般或特殊特征和品质。 （1911 年：129）
...
约翰森（1911 年）表达的保守主义关于确定基因的物质基础，作为基因型共享的生殖细胞的性质，并不像那些在那个十年里迅速采纳新术语基因、基因型和表现型的孟德尔研究人员那样明显。焦点转移到超越约翰森所称的“传递观念”遗传学，转向遗传学作为基因以新意义从父母传递到后代的生殖细胞（图 3）。实验室遗传学和农业育种的研究成功地延伸了孟德尔方法，但也使约翰森在第 2 节中引入的一些概念和方法问题持续存在并扩散。
...
一些孟德尔研究者将对基因的物质基础的调查扩展到它们在发育过程中的作用。例如，果蝇的眼睛通常是红色的，有时是白色的。遗传学家确定了染色体上对应于白眼突变的位置（Morgan 1919），后来调查了色素形成代谢途径以及参与果蝇眼睛发育正常或突变颜色的酶（调节生化相互作用的蛋白质）（例如，Beadle＆Ephrussi 1936）。二战后被称为分子遗传学或分子生物学的研究继续鉴定 DNA 作为基因的化学基础以及 DNA 复制、突变、转录为 RNA 和翻译为多肽（蛋白质组成部分）的机制。研究人员探究了调节这些机制的反馞网络，首先在病毒和细菌中，然后在复杂的多细胞生物中；绘制并修改了生物体的特定 DNA 序列；比较了分类群体中的序列（即，生命分类的不同分支中的群体），以评估种群中的遗传变异程度，并将分类群体分类为系统发育；追踪特定基因在发育过程中何时何地活跃；并研究了与基因不相关的 DNA 序列的作用（Griffiths＆Stotz 2013）。这类研究现在占据了生物学的中心，使许多研究人员和评论员认为，随着时间的推移，对生物体的已识别 DNA 变异的影响的组合将最终有助于理解特征的发展（请参阅基因条目）。

## 4. 通过关注生物材料和条件的控制引发的哲学问题

正如前文所述（并在图 2 的方法与图 1 的理论之间的对比中传达），约翰森没有提供将自然变异种群划分为表现型作为生物体类别的方法，更不用说使用这些类别来识别基因型作为这些种群内的类别。那么，为了在自然变异种群的遗传研究中应用他的术语和区分，需要什么呢？可以勾画出许多途径：

1. 重新整合-开发方法，通过控制实验中使用的生物材料和条件，将原基因型和表现型的呈现作为基础，并用于遗传研究的后续发展；
2. 工程师-在越来越广泛的背景下保留实验控制；
3. 在理论上概括-使用支持控制和工程的理论和模型作为更广泛解释和/或干预的基础（Hacking 1983；Cartwright 1999；有关科学理论结构的条目）；
4. 类似-认为或行动，就好像自然变异人口或较少受控情况中的观察源自类似的理论和模型；和
5. 进行更多实验-不要追求重新整合，而是继续采用孟德尔方法来了解有关生物的更多信息（Waters 2004）。

作为社会学问题而非逻辑问题，工程成功可能支持理论概括，二者可能反过来使任何假定的扩展到自然变异人口更加合理。伴随进一步实验，这些途径最终可能导致重新整合的成功。可以想象，在受控条件下暴露的过程最终可能解释自然变异人口中的遗传。这可能是通过研究人员识别由父母传递给有机体的基因型的物质成分，然后追踪所有这些成分如何随着时间的推移影响有机体特征或表现型的发展-也许首先在受控条件下，然后在不同的生态情况下（参见生物学中还原主义的条目）。然而，并没有保证基因型-表现型区别的原始实验基础或随后的发展必须导致有效的工程、理论概括或类似，以澄清。事实上，作为社会学问题而非逻辑问题，追求这些步骤可能会分散对重新整合项目的注意力。第 5 节回顾重新整合所需的内容，以此来质疑基于原始实验的区别作为研究自然变异人口遗传的基础的地位。第 4 节的其余部分指出了几个哲学讨论领域，这些领域是由上述途径和步骤引入并延伸的实验带来的。

“新实验主义”通过研究实验室中发生的事情或更一般地说，通过研究实现可靠结果的实际方法，对科学哲学中对理论的传统强调提出了反驳或复杂化（参见生物学实验和科学理论结构条目）。正如前文所述（第 2 节），同样需要关注的是生物学领域，比如遗传学研究，首先是如何成为实验性的。生物学实验可能导致新现象或对象的工程化，比如敲除小鼠（即具有特定基因失活的系列），但同时也留下了一个问题，即通过实验中使用的生物材料和条件的控制而被弱化的东西的重要性。继续敲除的例子：在高度近交小鼠系列中敲除的基因的效应是否可以推广到其在自然变异的小鼠种群，更不用说其他物种中的效应？换句话说，在敲除线中展示具有定义效应的基因可能是某种被代表的东西的教科书案例——DNA 序列作为基因——由于其缺失的可靠效应而被赋予真实地位。然而，反现实主义者可能指出，鉴于孟德尔研究和随后的分子生物学的特殊实验条件，尚未观察到的东西（参见科学现实主义条目）。尽管存在这样的反对意见，如果存在一种能够产生结果的方法，即使结果不能解决曾经激发他们探究的问题，仍会有科学家应用它（例如，正如前文所述，遗传学研究开始关注差异而不是相似性和发展）。任何这种实用主义如何被看待？—生物学实验是否像哲学实用主义一样关注真理，还是关注实现目标并制定可以在实践中追求的进一步目标（参见实用主义条目）。或者，这是否更多地突出了社会学而不是科学哲学中的实用主义，其中研究人员或科学解释者考虑在实践中修改已被确立为知识的难度（Latour 1987）？最后一种意义与本条目对现代实验生物学和农业育种中生物材料和条件控制的实质实践形式的抽象的关注相契合。然而，应该指出的是，这种抽象形式集中于具体的对象，因此不符合对抽象对象与具体对象的对比（参见抽象对象条目）。

当理论和模型扩展到较少受控情况时，如农业育种试验以及从中得出的数据分析，与实用主义、科学现实主义和抽象相关的问题变得更加重要。正如前文所述，数量遗传学依赖于对未观察到的理论基因型贡献的模型。使用这些模型对数据进行分析使育种者能够决定通过选择增强哪些特征，尽管他们没有独立于数据以确认其模型中关于理论基因型及其贡献的假设的证据（Lloyd 1988）。然而，随着出版物、职业生涯、品种发布、软件包等建立在这样的基础上，从实际角度来看，研究人员很难提出不依赖于未观察和未确认的实体和属性的替代方案。事实上，未构想到的替代方案，Stanford（2006）强调的可能性，很可能包括因各种原因不切实际的方法的理论。需要实用方法的实用问题反过来适用于哲学：当哲学家区分或指出科学家未澄清或未深入研究的问题时，他们打算通过何种方式影响科学家改变他们的观点或实践？这个问题由本条目留待开放。基因型-表现型区别已在本条目中定位于对生物材料和条件的控制，从而引起对通过该控制被弱化的东西重新整合的挑战的关注。然而，哲学家没有提供任何方法来使这一挑战超越描述——框架——将是一个有益的起点的暗示。换句话说，本条目将基因型-表现型区别定位于新实验主义中对科学实践的描述性强调、重新整合的规范可能性以及关于转变实际实践所需方法的一个开放问题。 （描述性与规范性观点的对比在 Stegenga 2009 年对韦伯对实验生物学哲学的贡献的评论中得到探讨。）

描述与规范对比也涉及到给予基因型-表现型区别不同含义的问题。哲学家应该描述性地追踪从约翰森到当今对该区别含义的转变，还是应该规范性地消除不同研究小组的工作中甚至在同一组内可能存在的不同含义（参见模糊性条目）？或者应该简单地描述，指出共存的含义使基因型-表现型区别成为一个“边界对象”，使各个领域（或“社会世界”）能够互动，尽管这些领域对术语的使用有不同的目的（Star & Griesemer 1989）？也许给予该术语不同含义的事实提醒了一旦理性和客观性被视为不是“普遍或必要[事项]，而是局部和偶然的，相对于科学兴趣和目的”时，科学的不一致性（统一科学条目；Cartwright 1999）会随之而来。描述性地，哲学家可以剖析使探究合理的不同、有时是不可比较的兴趣和目的。规范性地，他们可能强调与某一领域的兴趣和目的不一致之处，但他们也可以倡导接受反映不同科学的多元化。

> 在与人类认知能力的局限性和各种实用兴趣在这些现象的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型的表现型

(The Social Dimensions of Scientific Knowledge; on “pragmatic interests”, see also references on social and economic history of heredity at the end of [section 1](https://plato.stanford.edu/entries/genotype-phenotype/#SettScenDiffKindMeanGenoPhen)). However, two other possibilities remain, namely, ambiguity in the use of the genotype-phenotype distinction obscures shortcomings in theories and methods and allows the advances in one field (e.g., molecular genetics) to be seen to render more plausible the empirically and conceptually unrelated claims of another (e.g., quantitative genetics) (see sections [5.2.1](https://plato.stanford.edu/entries/genotype-phenotype/#QuanGene), [6.4](https://plato.stanford.edu/entries/genotype-phenotype/#WhatCounUndeInsiObseSurf), and [6.5](https://plato.stanford.edu/entries/genotype-phenotype/#FindAbouHereDeriUsinOneMeanTermAffiFindDeriUsinDiffMean)).

## 5. Control and Reintegration

第 2 节和第 3 节描述了如何在特殊受控条件下操作化原始的基因型-表现型区别，即，在统一条件下培育和交配纯合系谱。第 4 节阐述了基于实验的区别的路径：重新整合，工程化，理论上概括，类比和进一步实验。然而，生物学和生物哲学并未强调需要重新整合已经被抽象化的内容，作为基因型遗传概念在超越这些特殊条件（图 2 和 3）并应用于研究自然变异种群遗传学时的必要步骤。因此，为了突出在受控条件下建立基因型-表现型区别的影响，本节考虑了迄今为止审查的不同领域中控制和可能重新整合可能涉及的内容。

### 5.1 实验遗传学

#### 5.1.1 纯合系谱

对于近交系，与自然变异种群的领域相比，表现型作为类别并不用于识别基因型作为类别；事实上，它被认为是一种表现型，因为给定了基因型，即近交系。就约翰森的实验所能辨别的而言，基因型作为物质成分可以是整个生殖细胞或种子。重新整合将涉及构想这些基因型作为整个细胞的成分的作用，并找到调查它们对生物体特征随时间发展的影响的方法（参见发育生物学条目）。这样一个计划有支持者，特别是在二十世纪上半叶，但被孟德尔遗传学所掩盖，并被遗传学家和哲学家所忽视（Sapp 1987）。

#### 5.1.2 孟德尔杂交

孟德尔实验需要比近交系更多的控制，因为这些系必须在统一条件下培育、杂交和自交。然后可以使用表现型来区分关于基因型的多个假设，其中表现型是一类仅分享有机体全部特征集合的一部分的有机体，而基因型是一类有机体，它们的生殖细胞的一部分是相同的。基因型的相关部分被显示为沿染色体上的基因对，只要考虑到孟德尔实验所涉及的控制，重点在于特征的差异，而不是后代如何发展出具有该特征。请记住，正如约翰森所指出的，孟德尔实验在检查种典型特征方面是有限的，以及随后的发展。再次，可以想象一个通过实验控制所抽象掉的重新整合计划：研究人员确定基因型的物质成分，然后追踪所有这些成分如何影响种典型特征随时间的发展。从这些影响的综合中，有可能形成一个结构完整的有机体。然而，遗传研究的两个新特征与这样一个重新整合计划相悖：如前所述，遗传已经等同于差异的传递和代际间模式。这意味着发展成为一个独立且次要的问题（Sapp 1987）；对形态结构的种典型发展动态的分析被遗传学所掩盖。

严格来说，声称孟德尔实验无法检查物种特有特征是不正确的。例如，果蝇属的所有物种的所有个体在头顶中线上对称三角形排列着三个简单的光感受器，即眼点。最简单的假设是，基因型（在物质成分意义上）对这一特征的影响没有变异，其发育对正常环境干扰具有抵抗力。然而，如果果蝇的发育受到足够的干扰，就会观察到一些具有两个或更少眼点的果蝇。如果那些眼点少于三个的果蝇被用作下一代的父母，它们会产生比父代更多的异常果蝇。当从异常果蝇进行选择育种的过程持续多代时，会产生一系列果蝇，即使在没有任何外部发育干扰的情况下，这些果蝇始终具有两个眼点（Maynard Smith & Sondhi 1960）。这种以及其他类似的选择实验的成功表明，该特征最初的一致性并不意味着在可能在某些条件下影响该特征发育的所有基因型中存在一致性（Rendel 1967）。现在，任何研究多样的基因型如何导致典型的三眼点图案的发育都必须解释异常图案的出现。

尽管约翰森（1911）几乎没有关注发育动态，但请回想（来自第 2 节），他希望将基因型视为一个整体，并认为“染色体的离散颗粒是‘承载’所讨论整体遗传的特殊部分”这一想法没有价值（1911: 131–2）。然而，孟德尔实验似乎表明了离散颗粒的想法是合理的。

#### 5.1.3 分子遗传学

通过分子遗传学的研究计划取得了恢复被抽象化的进展（如第 3 节末总结）。这并不是说任何生物体的基因和 DNA 变体的目录或数据库远程类似于其发展的字面“蓝图”或“程序”。然而，随着对分子水平遗传学的知识和操纵 DNA 技术的不断改进，遗传学领域现在不仅涉及解释一个生物体在特征上与另一个生物体的差异，还涉及阐明基因活动和反馈（基因调控）网络以及有机结构的主要分支点的发展——生化、生理和行为，以及形态学——表现型作为特征的变体。这种进展是否最终导致对基因型作为整体运作（甚至对基因型的某些有限部分）的解释，以及随后导致结构的物种典型发展，尚待观察（Robert 2004 年，发展生物学条目）。然而，通常会忽视这种重新整合的需求。例如，当假定源自共同祖先的基因（同源基因）应该在从该祖先起源的分类群中具有相同功能并影响相同特征时，这一点就显而易见了（例如，PAX6 基因与哺乳动物和果蝇的眼睛有关）。这种假设忽视了特征可能依赖于整体基因型在整个生物体的发育中以及基因可能通过在进化谱系中转变的角色而保守的可能性。

尽管分子遗传学取得了进展，其方法涉及另一种重要的控制形式。分子遗传学中典型的统一条件排除了生态背景中发展的动态，而生物体及其特征却有助于塑造这种背景（Gilbert＆Epel 2009; Lewontin 1983）。可以想象，在严格控制的情况下对纯种生物体的遗传特征或至少在特征之间的差异进行遗传学调查，可能会成功地推断到生态背景中特征的发展（例如，在研究生长和繁殖策略以及环境变化下特征的可塑性[“表型可塑性”；发育生物学]的调查中）。然而，这些自然变化情况方面的重新整合需求往往会被忽视。考虑作为人类医学模型的动物实验。人们经常对将小鼠作为人类模型的有效性提出质疑。然而，即使谈论“小鼠”和“人类”也是一种抽象，忽视了小鼠之间的变异和人类之间的变异。如果关注变异，首先要注意的是实验室小鼠高度选择的品系比未驯化种群变异性小（Rader 2004），在这些小鼠上进行的实验涉及严格控制的情况。然后可以问，在从不同和更加复杂的情况中培育的未驯化种群的���体中，实验观察到的机制在多大程度上依赖于通常在描述机制时未列举的因素的控制价值？当生物技术扩展其控制条件的能力并利用基因工程生物生产所需产品时，这种质疑就被排除了。（生物技术可以被视为分析生物学的工业表现，这个计划旨在通过将其切割成一些适当的小部分来理解生物体。在这里相关的是生物技术兴起的政治、经济和文化维度，以及在农业、健康、食品科学、法律体系等领域中遗传学本身的兴起。所有这些都超出了本条目的范围；请参阅 Müller-Wille＆Brandt 2016。）

### 5.2 选择育种模型的使用

在数量遗传学和种群遗传学中使用模型也是基于对生物材料和条件的控制。对于这些领域，正如本节所示，更难制定重新整合曾被弱化的方案。

#### 5.2.1 数量遗传学

孟德尔实验通过杂交纯合系达到了约翰森（以及跟随他的生物学家）的目标，即提供可重复的结果并揭示隐藏的过程，但是数量遗传学旨在分析连续性状（参见 3.3 和 3.4 节），与该目标存在模糊的关系。模型使育种者能够预测不同交配设计下的结果；由于实际实现的结果通常与预测的结果有所不同，因此这些结果并不严格可重复。此外，虽然研究人员可以想象隐藏的过程类似于模型中的理论过程，但是作为数量遗传模型基础的理论基因型是不可观察的（参见科学现实主义条目）。然而，对材料和条件的控制实践确保了基于模型的分析继续是有用的。特别是，当结果与预测之间存在差异时，这可能是由于隐藏的过程与模型中的理论过程不同，育种者总是可以进行补偿：他们可以淘汰不受欢迎的后代，从理想的后代中进行繁殖。随着建立在模型基础上的育种计划的制定（例如 Holland 等人 2003 年），在实践中实施从另一种模型构建的数据分析变得越来越困难（参见实用主义和第 4 节）。请注意，由于数量遗传学涉及对连续性状数据的统计分析，因此必须能够使用避免引用未观察到的内容的模型来分析性状变异的数据（Taylor 2012）。然而，数量遗传学并没有追求这种可能性（因此成为斯坦福[2006]早期提到的未构思的替代方案的潜在案例）。实际上，将任何替代数量遗传模型应用于人类性状变异数据的分析困难重重，即使在那个领域，育种并不是一个选择，对生物材料和条件的控制也是最小的。

如果在实践中难以实施基因型观察和数据分析的育种计划，而这些计划避免参考未观察到的内容，那么很难构想没有以下规范消歧的替代方案。在这里，可以考虑更多的是恢复而不是重新整合，也就是坚持实际可观察的基因型和基因型方差之间的区别（有时缩写为遗传方差，其中方差是给定数量的变化的统计量度）。后一术语源自育种者使用基因型值一词来表示在他们培育或种植基因型的所有地点中某一特征的平均值（指的是由共同祖先或祖先集合血统相关的个体类别；请参见第 3 节）。这些基因型值之间的变化，简称为基因型方差。换句话说，这一数量源自于对相关和无关个体在其表现型（指观察到的特征）而非基因型（指 DNA）中的变化进行统计分析。基因型方差不仅随着基因型和地点的混合而变化，而且其统计估计不涉及影响特征发展的可测量的遗传或环境因素。不幸的是，研究人员和评论员，包括哲学家，通常会谈论基因型或遗传方差，比如“遗传差异对个体间观察到的差异的贡献”（Plomin 等人，1997 年：83），好像特征的变化和基因型的变化之间有一些明显的关系。这种混淆可能源自于定量遗传模型是基于基因型（指基因对）。但那些是理论基因型，未观察到的，并且如上所述，对于特征变异的分析并非必要。 （为了增加混淆的可能性，基因型方差与所讨论的特征中观察到的总方差之比的技术术语是遗传力，这与父母和后代特征之间通过基因传递存在联系的存在无关[Taylor，2012 年]。）

为了增强消歧义，Taylor（2012）建议使用在农业试验中熟悉的术语：品种代替基因型（指由共同祖先或祖先集合血统相关的个体类别），地点代替环境，特征代替表现型（考虑到最后一术语意味着与一组基因型有关，指基因对）。无论这些替代术语是否被更广泛采用，一旦从定量遗传学的统计模式和可测量的潜在因素之间的差距被认识到并持续观察到，就很难理解那些混淆或在“遗传”不同含义之间滑动的描述的推理。这种困难延伸到忽视基因或基因型与环境之间相互作用的描述，然后继续，好像在定义了变异的定量遗传学分析中，相互作用与测量到的基因存在和环��变量之间的统计相互作用有某种概念或经验联系。 （本条目的范围不包括回顾科学家或哲学家的这类描述；请参见 Taylor，2015 年以及第 6.5 节中关于混淆或模糊含义的讨论。）

有一种恢复基因型方差与实际基因型变异之间区别的替代方案，即专注于后者。随着在基因组的任何一段 DNA 序列（即基因型作为物质成分）的确定已经变得常规化，全基因组关联（GWA）研究允许估计与可测量遗传变异相关的特征变异的部分。 （研究的变异是单核苷酸多态性[SNP]，这些变异被认为不是影响特征的 DNA，而只是在基因组上与这些因素接近的某处。）然而，事实证明，即使一起检查许多遗传变异，只有很小一部分特征变异与遗传变异相关，或者用统计术语说，“由”遗传变异“解释”（McCarthy 等人，2008 年）。这一发现引发了有关遗传缺失的讨论（例如，Manolio 等人，2009 年）。然而，这种新遗传性与定量遗传学的遗传性没有概念上或经验上的联系。在使用术语遗传性时存在的额外模糊程度，与高遗传性意味着遗传变异可能解释大部分特征变异的未实现期望相关，前面段落中确定的恢复是合理的。

一种关于基因型差异和实际基因型差异之间区分的合理解释是 GWAS 研究成功有限的一个可能原因。即使经典数量遗传遗传力很高，双胞胎或一组近亲的相似性与尚未确定的基因型或遗传因素的相似性相关联，这些因素可能不同于下一组亲属，或者不同于下一个位置（环境）。换句话说，潜在因素和它们影响的发展途径可能是异质的。可能是一对等位基因，比如 AAbbcbDDee，受一系列环境因素，比如 FghiJ 的影响，与等位基因 aabbCCDDEE 受一系列环境因素 FgHiJ 的影响，所有其他条件相同，可能导致相同的结果（Taylor 2012）。显然，如果生物材料和位置接近原始亲属和环境因素，那么在特征相似性背后存在异质因素的可能性就会减少。推论是，当数量遗传模型的使用者忽视这种可能性时，实际上是假设对生物材料和条件有严格控制。

通过定位数量性状位点（QTL）的技术，对经典数量遗传模型中不可观测的部分进行了一定程度的重新整合——这些位点是基因组中包含与连续可变性状相关的遗传因素的区域。然而，QTL 定位在动植物品种中取得了最大的成功，这些品种可以在受控条件下复制和培育；对人类群体的可靠 QTL 结果很少（Majumder & Ghosh 2005；但参见 Wellcome Trust Case Control Consortium 2007）。事实上，在数量遗传学中，将发展的复杂性重新整合到其生态背景中，这些复杂性在模型的参数中被简化，对于人类案例来说，这方面的发展仍然很少（Turkheimer 2004）。

#### 5.2.2 种群遗传学

在种群遗传学中，发育的复杂性及其生态背景也被折叠成模型的参数，比如用于生存和留下后代的参数。因此，当种群在实验室中受到人工选择时，对基因型（即基因对）的选择系数的估计更容易进行，而当在野外观察频率或随时间变化时，情况就不那么明朗了。Lewontin（1974a）提出了对某一天恢复那些选择系数中所抽象掉的东西的可能性表示怀疑的理由。Lewontin 总结说，只有当单个等位基因替换与性状有很大差异时，才可能测量选择系数和模型的其他参数，而当基因替换的效应只造成很小差异时，就不可能了。这使他评论道：“我们能够测量的东西在定义上是无趣的，而我们感兴趣的东西在定义上是无法测量的”（1974a: 23）。当存在多个连锁位点时，将种群遗传模型与观察结果相关联的问题变得极为严重（1974: 317）。他建议，种群遗传学应将注意力转向染色体长片段的选择系数。这一计划，就像约翰森希望将基因型视为一个整体一样，几乎没有被追求。即使追求了，自然种群中变异的生态遗传学分析，以及其中的所有复杂性（例如，Clausen 等人 1948 年的研究），仍然需要重新将生态背景融入种群遗传学中。

## 基因型-表现型区别的意义是什么

原始术语基因型、表现型及其之间的区别的特殊实验条件也对约翰森的遗传基因型概念的采纳所解决的问题产生了影响。

### 6.1 障碍物与传递至下一代的特征之间的关系

到达下一代的 DNA 在生物体一生中发生的大部分相互作用（包括生物体内部和与环境的相互作用）的影响下被屏蔽，这就形成了“特殊”特征（Johannsen 对父母特定发展过程中获得的特征的称呼）传递给后代的障碍。 （这种屏蔽的关键部分是 DNA 向 RNA 的单向转录[然后编码成构成蛋白质的氨基酸]，而不是一般的反向转录。）虽然基因型-表现型区别可以被视为存在这种障碍的标志，但研究人员声称展示绕过这种障碍的方法已有悠久历史。最值得注意的是，现代表观遗传学科学，借助对 DNA 序列和基因功能的信息不断增加的了解，展示了细胞外化学物质如何修改一个生物体的基因活动，有时甚至延续到后代（Stotz 2006）。

发育系统理论（Oyama，Griffiths 和 Gray 2001），进化发育生物学（“evo-devo”）（Moczek 等人 2015），以及行为和认知研究中的后基因组综合（Stotz 2008）也主张关注扩展遗传，这包括不仅是表观遗传修饰的传递，还有生物体外资源的传递，例如爬行动物在确保卵孵化的温度适宜的地方产卵。最后一个例子也适用于生态位建构理论（Odling-Smee 等人 2003）。这一领域研究了生物体塑造其发展特征、生存和繁殖的生态背景的重要性，如河狸生活在它们建造和维护的水坝形成的池塘生态系统中的例子。 （表观遗传和生物体外资源是否具有与基因相同的因果地位是一项超出本条目范围的哲学辩论问题；请参阅 Waters 2007；Stotz 2006；Griffiths 和 Stotz 2013；遗传系统条目，以及下面的第 7 节。）

### 6.2 进化定义为基因频率的变化

生物进化，在其最一般的解释中，是通过后代的修改，即，随着时间的推移，从共同祖先衍生出的种群或分类群中观察到的特征或形式的频率发生变化。然而，随着遗传基因型概念的兴起，出现了更狭窄的定义，即，只有基因频率的变化才被认为是进化正在发生。所谓后天特征的遗传被认为是无关紧要的，任何形式的变化都没有基因频率的变化。支持扩展遗传的人不同意，他们在进化的定义中包括了发育系统和其构建的生态位的变化（Griffiths＆Stotz 2013）。对这种分歧的裁决在一定程度上取决于数量问题：除了基因型（指整个基因组的 DNA）之外的资源在影响生存和繁殖成功的特征发展中有多重要？重要性可能取决于资源传递的代数。然而，如果注意力放在发展动态上，那么这种分歧就变得质的，甚至是激进的，这种发展动态是在展示遗传基因型概念时被抽象化的生态背景中重新整合的。重新整合发展动态不仅仅是注意到发育噪声的存在，例如当果蝇个体在每一侧的鬃毛数量上不对称时。下一小节将详细阐述。
