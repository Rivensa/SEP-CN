...
在证明论中（特别是在这个学科分支中，即序数分析），一个基本主题是通过超限序数对理论进行分类，这些超限序数衡量了它们的“一致性强度”和“计算能力”。这些序数给出了一个理论有多强的指示，并因此提供了比较不同理论的方法。例如，序数 ε0 是 Peano 算术的证明论序数，远小于序数 Γ0，通常称为“可预测性的极限”（见上文 1.3 节）。这表明，有可预测接受的理论比 Peano 算术要强得多。
