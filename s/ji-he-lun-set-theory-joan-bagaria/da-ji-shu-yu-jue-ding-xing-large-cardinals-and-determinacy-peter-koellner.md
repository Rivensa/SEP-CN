...
现在让我们考虑一些例子。首先考虑底层 Peano 算术（PA）理论的自然数概念。这个自然数概念不仅为 PA 语言的数学归纳提供了正当性，还为任何有意义的 PA 语言的数学扩展提供了正当性。例如，如果我们通过添加 Tarski 真理谓词来扩展 PA 语言，并通过添加 Tarski 真理公理来扩展 PA 公理，那么基于自然数概念，我们有正当理由接受涉及真理谓词的数学归纳的实例。在得到的理论中，可以证明 Con（PA）。这个过程可以迭代进行（有关此主题的更多信息，请参见 Feferman（1991）及其引用）。相比之下，“ZFC +“存在 ω 个 Woodin 基数””的 Π01-陈述（我们将在下文中讨论）无疑不是基于自然数概念本质上的正当化；相反，它的正当化源于当代集合论中复杂的定理网络，我们将在下文中讨论。

考虑集合的迭代概念。与算术的情况类似，这个概念（可以说）本质上为集合论语言的某些扩展的 Replacement 和 Comprehension 实例提供了内在的理据。但是，基于这个概念，还有更丰富的原则（可以说）本质上是有理据的，即集合论反射原则。这些原则大致上断言，任何在 V 中成立的属性也在某个初始段 Vα 中成立。这些原则产生了不可达和马洛基数（以及更多），很可能是哥德尔在上述段落中的主张的基础。有人认为，这种反射原则基于集合的迭代概念本身就是有理据的，并且它们穷尽了可以在这个概念上本质上有理据的原则（例如，参见 Tait（2001），收录于（2005b），第 283-284 页）。
...
对于一个点类 Γ（k-元组的）实数集合，令 Det(Γ)为 Γ 中所有实数集合都是确定的陈述。在接下来的内容中，我们将假设 Γ 具有某些最小闭包性质；更确切地说，我们将假设 Γ 是 Moschovakis（1980）意义下的适当点类。这个概念的具体细节对我们的目的来说并不重要；可以说迄今为止引入的所有点类都是适当的。
