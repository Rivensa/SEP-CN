...
康托尔将两个集合定义为具有相同基数的集合，如果它们之间存在一个双射。在有限领域中，这当然是常识；他的独创性在于将其扩展到无限领域，并且不回避表面上悖论的结果。在无限领域中，基数和序数不是同构的概念，就像在有限领域中一样：一个序数类型为 ω 的良序（比如自然数上的通常顺序）和一个序数类型为 ω+ω 的良序（比如将所有奇数放在所有偶数之前，并按照通常顺序放置奇数和偶数的集合）代表不同的序数，但它们的域（作为相同的集合！）当然是相同大小的。康托尔将自然数和完全平方数之间的表面上的等势性（由伽利略注意到）以及不同半径同心圆上的点之间的一一对应（自中世纪以来就被注意到）视为关于特定无限集等势性的事实证据。
...
弗兰克尔添加了替代公理方案，以便能够构造更大的集合（即使在策梅洛集合论中也无法证明 ℵω 的存在）。基本思想是，与集合大小相同的任何集合都是一个集合，可以逻辑上表述如下：如果 `ϕ(x,y)`是一个函数公式 `∀x∀y∀z[(ϕ(x,y)&ϕ(x,z))→y=z]`，而 A 是一个集合，则存在集合`{y∣∃x∈A(ϕ(x,y))}`。
...
强集合公理方案断言，如果对于每个 x∈A，存在 y 使得 ϕ(x,y)，那么存在一个集合 B，对于每个 x∈A，存在 y∈B 使得 ϕ(x,y)（与通常的方案相同），但是对于每个 y∈B，存在 x∈A 使得 ϕ(x,y)（B 不包含任何冗余元素）。由于分离公理的较弱形式，附加的限制是有用的。

子集集合方案可以被看作是包含了一个非常弱的幂集。它断言，对于每个公式 ϕ(x,y,z)，对于每个 A 和 B，存在一个集合 C，对于每个 z，使得 `∀x∈A∃y∈B[ϕ(x,y,z)]`，存在 Rz∈C，对于每个 x∈A，存在 y∈Rz 使得 ϕ(x,y,z)，并且对于每个 y∈Rz，存在 x∈A 使得 ϕ(x,y,z)（这与强集合公理中的限制相同；注意不仅构造了关系下的映像，而且将这些映像进一步收集到一个集合中）。
...
分离，替代：与我们上面对 ZFC 的介绍相同，只是标准性谓词不能出现在公式 ϕ 中。

定义：对于任意公式 ϕ，通过将宇宙上的每个量词替换为对所有标准对象的量词（并将集合中的每个有界量词替换为对该集合的标准元素进行限制的量词），得到公式 ϕst。

理想化：存在一个包含所有标准集合的有限集。

转移：对于每个不涉及标准性谓词且不包含除标准集合之外的参数（自由变量 x 以外的变量）的公式 ϕ(x)，`∀xϕ(x)≡∀x(standard(x)→ϕ(x))`。

标准化：对于任意公式 ϕ(x)和标准集合 A，存在一个标准集合 B，其标准元素恰好是满足 ϕ(x)的 A 的标准元素。

我们的理想化形式比通常的版本更简单，但效果相同。

转移立即意味着任何唯一可定义的对象（不涉及标准性的定义）实际上都是标准对象。因此，空集是标准的，ω 是标准的，等等。但并非所有标准对象的元素都是标准的。考虑包含所有标准对象的有限集合的基数；这显然大于任何标准自然数（ω 的通常元素），但它显然是 ω 的一个元素。事实证明，每个元素都是标准的集合都是一个标准有限集合，这是可以证明的。
...
反射公理：对于每个宇宙 V，存在一个更高的宇宙 W，其中存在一个序数 θ，使得 V 在元素上等同于（或同构于）Wθ，W 中的累积层次结构的一个层次。
...
这个理论是我们的一个提议，详细阐述了 Rudy Rucker 的建议。我们（和许多其他人）观察到，在康托尔的天堂中，只有两个无穷的顺序实际上在集合论之外的经典数学实践中出现：这些是 ℵ0 和 c，自然数的无穷和连续的无穷。Pocket 集合论是一种理论，其动机是这些是唯一的无穷（顺便说一句，Vopenka 的替代性集合论也具有这个特性）。
...
类的概括：对于任意公式 ϕ，存在一个类`{x∣ϕ(x)}`，其中包含所有满足 ϕ(x)的集合 x。（注意，这是 Kelley-Morse 集合论的类概括公理，没有对 ϕ 中的量词施加任何限制）。
...
存在一个无限序数，因为存在一个可以与一个人喜欢的无限集 I 进行一对一对应的序数。由于存在一个无限序数，每个有限序数都是一个集合，第一个无限序数 ω 也是一个集合。由此可知，所有无限集都是可数无限的。

无限集 I 的幂集与 Cantor 定理不具有相同的大小，肯定是无限的，因此不能是一个集合，因此必须与宇宙的大小相同。根据通常的考虑，宇宙与 ℘(ω)或 R（以任何通常的方式定义的实数集）具有相同的大小，其“基数”为 c。此外，第一个不可数序数 ω1 是宇宙的基数，因此连续统假设成立。
...
类的存在：如果 ϕ(x)是任意的公式，那么所有满足 ϕ(x)的集合 x 构成的类 ϕ(x)存在。（集合 x 与 x 的元素类等同）注意，Kuratowski 对的集合是集合，因此我们可以像通常一样在集合的宇宙上定义（类）关系和函数。
...
定义：类型为 ω 的排序是一个类的良序，它是无限的，并且其所有初始段都是有限的。如果一个类具有类型为 ω 的排序，则它是可数的。

类型为 ω 的排序与标准自然数具有相同的长度。我们可以证明存在这样的排序：考虑有限（即标准有限）的冯·诺伊曼序数上的顺序。必须存在无限的冯·诺伊曼序数，因为冯·诺伊曼序数与整个集合宇宙之间存在一个集合论可定义的双射：任何适当的半集合都可以转换为冯·诺伊曼序数集的适当的半集合。
...
这意味着（如在袖珍集合论中），存在两个无限基数，可以看作是 ℵ0 和 c，尽管在这个背景下它们的行为不如在袖珍集合论中那么熟悉。例如，所有自然数的集合（如 Vopenka 所定义的）的基数是 c，而自然数的一个初始段（有限自然数）的基数是预期的 ω。
...
这是一个相当困难的理论。在通常的集合论中，替代性集合论的模型是一个大小为 ω1 的 Vω 的非标准模型，其中每个可数的外部函数都可以扩展为模型中的一个函数。最好假设这个模型是在 L（可构造宇宙）内构造的，以满足基数公理。外延编码公理可以从环境集合论中的选择公理推导出来。
...
Andrzej Kisielewicz（1998）最近提出的一个建议是，通过使用两个不同的成员关系 ∈ 和 ε，可以规避集合论的悖论，每个成员关系用于定义另一个的扩展。

我们提出了公理。这个理论的原始概念是相等（`=`）和两种成员关系 ∈ 和 ε。如果一个公式 ϕ 是统一的，那么它不提及 ε。如果 ϕ 是一个统一的公式，那么 ϕ∗ 是相应的公式，其中 ∈ 被 ε 替换。如果集合 A 是规则的，那么它在两个成员关系上具有相同的扩展：x∈A≡xεA。

集合论的理解公理断言，对于任何统一的公式 ϕ(x)，其中所有参数（除 x 之外的自由变量）都是正则的，存在一个对象 A，我们使用符号`{x∣ϕ(x)}`表示，使得 `∀x((x∈A≡ϕ∗)&(xεA≡ϕ))`。
