...
再次强调，为了避免循环，ϕ 中不能自由地使用 B。这要求为了进入 B，x 必须是现有集合 A 的成员。可以想象，这需要一系列额外的集合存在公理，如果 NC 仍然有效的话，这些公理都是不必要的。

ZA 如何避免罗素悖论？起初，人们可能认为它无法避免。毕竟，如果我们让 A 为 V-整个集合的宇宙-而 ϕ 为 x∉x，似乎又出现了矛盾。但在这种情况下，所有的矛盾只表明 V 不是一个集合。所有的矛盾只表明“V”是一个空名字（即它没有指称，V 不存在），因为泽尔梅洛系统的本体论仅包括集合。
