...
RCA0 的现代形式首次出现在 Friedman、Simpson 和 Smith（1983）的一篇有影响力的论文中，其中包括了作为有序半环的 N 的 PA-公理、递归包容公理方案和 Σ01 归纳方案。到 1983 年，反推数学已经基本上成为我们今天所知的样子，甚至包括了 Friedman 在 Stephen Simpson（2009: 35）组织的 AMS 特别会议上创造的“反推数学”口号的使用。
...
其中 φ 是算术的，X 不是自由的，尽管 φ 可能包含其他自由的集合和数值变量。ACA0 是通过将算术概括方案添加到 RCA0 公理中而得到的系统。ACA0 证明了算术归纳方案，即将归纳方案（Π1∞-IND）限制在算术公式上。这意味着 ACA0 证明了一阶 Peano 算术 PA 的所有公理。根据 Friedman（1976）的保守性结果，这个结果的一种逆结果也成立：在 ACA0 中可证的一阶算术语言句子也可在 PA 中证明。由此结果可知，Gentzen 对 PA 的一致性证明也适用于 ACA0，并且 ACA0 具有与 PA 相同的证明论序数，即 ε0。
...
更深入和微妙的论证线索源于保守性定理本身的地位。仅仅因为 WKL0 在 PRA 上是 Π02-保守的这一事实并不足以建立一个真正的有限主义归约，因为为了诉诸于它，保守性结果必须从有限的立场可证明。这个反对意见可以通过诉诸于 Sieg（1985）的定理部分反驳，该定理表明存在一个原始递归的证明转换，可以将任何 WKL0 对 Π02 句子的证明转换为对相同句子的 PRA 证明。如果我们接受 Tait 的论点，那么证明转换是一种从无穷的证明中产生有限证明的有限过程，正如 Hilbert（1928）所预期的那样。
...
然而，克雷泽尔的提议似乎仍然包含一个反推的成分。任何超算术良序都可以被预测地定义为自然数的子集，因为根据斯佩克托（1955）的结果，这样的良序实际上是可计算的。然而，良序实际上是良序的这一事实通常是无法预测地确定的：作为良序是一个 Π11 属性，因为它涉及对良序中所有可能的无限递减序列进行量化，或者等价地，对良序的所有子集进行量化。实际上，所有可计算良序的代码集合，克里尼的 O，是 Π11-完全的，因此是一个典型的反推对象（克里尼 1955：207）。这个担忧已经被克雷泽尔（1960：27）注意到，并且对于哪些可计算线性序可以被预测地证明为良序的研究思路导致了 20 世纪 60 年代的一系列进展。
...
一旦可证性作为对待被认为是可预测的序数 α 的条件出现在图像中，就需要形式系统，其中这样的 α 控制着可预测定义的允许迭代的长度。这导致了分层分析的形式系统的超限进展的发展。与序数 α 相关联的系统 RAα 对于每个 β≤α 都有变量 Xβ1,Xβ2,…，并且有公理断言在分层分析层次的第 β 级集合对于所有 β≤α 都存在。

然后可以在此设置中定义可预测序数如下。如果 α 是可预测序数，≺ 是（编码一个）线性排序，β 是 ≺ 的序数类型，则如果 RAα⊢WO(≺)，则 β 是可预测的。这有时被称为自治条件。因此，可预测序数是通过从 0 开始，并证明越来越大的序数在更强的形式系统 RAα 中是可预测的，其中 α 已经在某个较弱的系统 RAβ（对于 β<α）中被证明是可预测的。Feferman（1964）和 Schütte（1965b, 1965a）独立证明了最小的不可预测序数是 Γ0，现在被称为 Feferman-Schütte 序数或可预测性序数。因此，我们可以将可预测地可证明的陈述特征化为那些可以在某个 RAα（对于 α<Γ0）中证明的陈述。

ATR0 的证明论序数也是 Γ0，就像所有分层分析的谓词系统 RA<Γ0=⋃α<Γ0RAα 的并集一样。此外，ATR0 在 RA<Γ0 上是 Π11 保守的，因此在 ATR0 中可证明的任何 Π11 句子 φ 也可以在某个 α<Γ0 的 RAα 中证明，换句话说，φ 将具有一个谓词证明。根据证明论中的还原计划，对于 α<Γ0，可以将在证明论上可还原为系统 RAα 的系统称为谓词可还原的。ATR0 不能谓词可还原，因为它只能在所有分层分析的谓词子系统的并集上进行证明论还原，并且可以证明任何 α<Γ0 的单个 RAα 的一致性。然而，由于上述保守性结果，ATR0 被认为是局部谓词可还原的，即 ATR0 的任何 Π11 后果都可以在一个分层分析的谓词子系统中恢复。
...
ATR0 相比之下，在描述性集合论、代数和其他数学领域证明了许多不能在个别预测性系统内证明的定理。预测主义者可以将其用作工具来证明 Π11 定理，然后通过保守性定理将其视为预测合法的。这个想法有一定的吸引力。

然而，这种观点容易受到与有限主义还原主义相同的批评，如伯吉斯（2010）所提出的。尽管预测主义者能够证明保守性定理，并且看到任何 Π11 定理都可以在某个 RAα 中证明，但这本身并不能使他们对该定理的预测可证性有保证，因为将预测可证性与在 RA<Γ0 中的可证性等同起来是一种外部分析，依赖于 Γ0 是一个明确定义的对象等事实，而根据费弗曼-舒特分析，预测主义者无法同意这一点。他们必须验证还原所给出的证明确实是预测可接受的，并在此过程中检查相关的 α 是否是预测序数。正如伯吉斯指出的，还原主义方法的优势不在于预测可证性的捷径，而在于通过算术超限递归公理方案进行推理所获得的某种概念上的优势。此外，似乎使预测还原主义成为有吸引力的前向数学结果本身也带来了一些问题。因为像完美集定理这样的陈述不仅可以在 ATR0 中证明，而且实际上等价于其特征公理，它们本身严格来说是非预测的。因此，只有它们的 Π11 后果可以通过还原获得，而不是定理本身。
...
反推数学的动机、方法、技术和问题的一般调查是 Shore（2010）。Montalbán（2011）提供了反推数学中的一些未解问题的列表，尽管读者应该意识到其中许多问题现在已经解决，并且自（Montalbán 2011）和（Shore 2010）发表以来，反推数学的研究方向有所转变。这些调查中有两个特别值得注意的领域被忽视了。第一个是针对 Π12 句子的更细粒度的可约性概念，例如 Weihrauch 可约性。关于 Weihrauch 可约性和可计算分析的最新调查是 Brattka，Gherardi 和 Pauly（2021），而关于 Π12 句子的可约性概念的更一般讨论可以在 Hirschfeldt 和 Jockusch（2016）中找到。第二个是高阶反推数学，它修改了第二阶算术的形式框架，包括第三阶和更高阶的实体，从而允许更直接地形式化许多分析、拓扑等数学对象。Shore（2010，2013）提出了使用更高阶递归论的高阶反推数学方法，但更有影响力的是 Kohlenbach 使用更高阶算术的方法（Kohlenbach 2002，2005）。在这个框架中已经证明了一些引人注目的结果。例如，Normann 和 Sanders（2019a）证明了 Cousin 引理和 Lindelöf 引理的三阶形式化都蕴含了完全的二阶算术。高阶反推数学的其他最新研究包括超限递归（Schweber 2015），分析定理的强度，如 Cousin 引理和测度积分的性质（Normann 和 Sanders 2019b），关于开集的定理（Normann 和 Sanders 2020），以及维数和拟紧性等拓扑概念（Sanders 2020）。
