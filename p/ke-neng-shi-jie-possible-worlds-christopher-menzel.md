...
|  |  | 所有约翰的狗都是哺乳动物：∀x(Dx → Mx)。 |
| -- | -- | ------------------------------------------- |

|  |  | 所有约翰的宠物都是哺乳动物：∀x(Px → Mx)。 |
| -- | -- | --------------------------------------------- |

由于这两个句子都是真的，它们具有相同的外延。因此，根据句子的经典替代原则，我们可以在错误的句子中用(2)替换(1)的出现

|  |  | 不是所有的约翰的狗都是哺乳动物：¬∀x(Dx → Mx) |
| -- | -- | ------------------------------------------------- |

结果是同样错误的句子

|  |  | 不是所有的约翰的宠物都是哺乳动物：¬∀x(Px → Mx)。 |
| -- | -- | ----------------------------------------------------- |

然而，当我们在真实的句子中进行相同的替换时

|  |  | 必然地，所有的约翰的狗都是哺乳动物：□∀x(Dx → Mx)， |
| -- | -- | ------------------------------------------------------- |

结果是这个句子

|  |  | 必然地，约翰所有的宠物都是哺乳动物：□∀x(Px → Mx)， |
| -- | -- | ------------------------------------------------------- |
...
|  |  | 对于所有可能世界 w，‘∀x(Px → Mx)’在 w 上是真的。 |
| -- | -- | ------------------------------------------------------ |

通过将（8）解释为量化、物质条件和上述原子子句，我们可以得出（6）当且仅当为真。

|  |  | 对于所有可能的世界 w，以及在 w 中存在的所有可能个体 a，如果 a 在“P”的 w-扩展中，则 a 也在“M”的 w-扩展中。 |
| -- | -- | --------------------------------------------------------------------------------------------------------------- |
...
|  |  | 对于每个世界 w，w 中的每个狗个体都是哺乳动物：∀w（Ww → ∀x（Ixw →（Gx → Mx）））。 |
| -- | -- | ---------------------------------------------------------------------------------------- |
...
|  |  | 不存在这样的世界 w，使得 Algol 存在于 w 中但不是某人的宠物：¬∃w(Iaw ∧ ¬Ta)， |
| -- | -- | ---------------------------------------------------------------------------------- |
...
|  |  | Algol 是一只宠物，但存在一个世界，其中存在一个与她不同的对应物：<br />*Ta*∧ ∃*w*(*Ww*∧ ∃*x*(*Ixw*∧*Cxa*∧ ¬*Tx*)). |
| -- | -- | ------------------------------------------------------------------------------------------ |

如 (15) 中所述，对基本属性的归属也可以通过对应物来解释：说 Algol 本质上是一只狗，就是说

|  |  | 在任何可能世界中，Algol 的所有对应物都是狗：<br />∀w(Ww → ∀x((Ixw ∧ Cxa) → Gx))。 |
| -- | -- | ------------------------------------------------------------------------------------ |
...
|  |  | 对于所有的 SOA w，如果（i）可能，w 发生，并且（ii）对于所有的 SOA s，要么（a）必然地，只有在 s 发生时 w 才发生，要么（b）必然地，只有在 s 不发生时 w 才发生，那么，在 w 处‘∀x(Gx → Mx)’为真。 |
| -- | -- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
|  |  | 存在一个可能世界 w 和一个个体 a，该个体在 w 中是具体的且是异类的。 |
| -- | -- | -------------------------------------------------------------------- |
...
|  |  | 存在一个可能世界 w 和一个 haecceity h，它在 w 中被实例化，并且与在 w 中的 Exotic 属性共同实例化。 |
| -- | -- | --------------------------------------------------------------------------------------------------- |
...
|  |  | s 是分子事实 f 的重组=def s 是一个分子事实，其构成对象和构成普遍性恰好是 f 的构成对象和构成普遍性。如果它不包含与 f 相同的事实，则 s 是 f 的非平凡重组。 |
| -- | -- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
|  |  | 事态 s 和 t 相同当且仅当它们包含完全相同的事态。 |
| -- | -- | -------------------------------------------------- |

鉴于此，我们有：

|  |  | （组合）实际世界=定义为包括所有简单原子事实和那些事实的合取的总体事态 T@的事实。 |
| -- | -- | ---------------------------------------------------------------------------------- |
...
|  |  | 实体 a 在（组合的）可能世界 w 中根本存在=def（i）a 是 w 包含的简单事实或（ii）a 是在 w 中根本存在的实体的组成部分或连接词。 |
| -- | -- | ----------------------------------------------------------------------------------------------------------------------------- |
